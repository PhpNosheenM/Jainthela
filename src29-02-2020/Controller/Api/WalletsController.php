...
class WalletsController extends AppController
{
  ...
  {
    parent::initialize();
	 $this->Auth->allow(['walletDetails','addMoney']);
  }
      ...
      if($isValidCity == 0)
      {
        ...
        if($this->request->is('post')) {
          ...
          if($isValidToken == 0)
          {
			...
			$wallet->narration ="Online Amount into wallet ";
           //pr($wallet);exit;
			...
			//Create Receipts
				...
				$Voucher_no = $this->Wallets->Receipts->find()->select(['voucher_no'])->where(['Receipts.city_id'=>$wallet->city_id,'Receipts.financial_year_id'=>$financial_year_id])->order(['voucher_no' => 'DESC'])->first();
				if($Voucher_no){
				...
				$receipt->narration ="Online Amount into wallet ";
