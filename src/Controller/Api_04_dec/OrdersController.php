...
class OrdersController extends AppController
{
	...
	{
		parent::initialize();
		$this->Auth->allow(['DriverOrderList','CustomerVerify','DriverOrderStatus','DriverOrderDetail','ItemWiseCancelOrder','CustomerItemWiseCancel','placeOrder']);
	}	
...
/*                 $delivery_charges=$this->Orders->DeliveryCharges->find()->where(['city_id'=>$city_id]);
	...
	public function placeOrder()
    {
      ...
      if ($this->request->is('post')) {
        ...
        $this->set(compact('success', 'message','order_no','orderMessage'));
        $this->set('_serialize', ['success', 'message','order_no','orderMessage']);
	}
	...
	public function CustomerItemWiseCancel()
	{
		...
		if(is_array($order_detail_ids) && !empty($order_detail_ids))
		{
			...
			//$query1 = $this->Orders->OrderDetails->find()->last();	
			//pr($query1);
