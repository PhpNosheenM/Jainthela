...
class AppNotificationsController extends AppController
{
    ...
    public function index()
    {
		...
		$server_path=$first_url;
		//$server_path='http://13.127.236.140';
		$user_id=$this->Auth->User('id');
        $city_id=$this->Auth->User('city_id');
        $location_id=$this->Auth->User('location_id'); 
        $state_id=$this->Auth->User('state_id'); 
		...
		if ($this->request->is(['post','put'])) {
			...
			$appNotification->location_id=1;
			$appNotification->created_by=$user_id;
			$appNotification->expiry_date=date('Y-m-d',strtotime($appNotification->expiry_date));
				...
				$item_variation_id=$this->request->data['item_variation_id'];
				$combo_offer_id=$this->request->data['combo_offer_id'];
				$parent_cat_id=$this->request->data['parent_cat_id'];
				...
				if($link_name=='product_description'){
					...
					$dummy_link='jainthela://'.$link_name.'?id='.$category_id.'&parent_cat_id='.$parent_cat_id;
            ...
            if ($banner_data=$this->AppNotifications->save($appNotification)) {
				...
				if(empty($web_image_error))
				{
					...
					if(empty($banner_data->image_app)) 
					{ $im1 =  ''; }
					else 
					{ //$im1 = $server_path.$img_url;
						$im1 = $banner_data->image_app;
					}					
					...
					foreach($tokens_data as $tkn_data){
						...
						if(!empty($app_token)){
							...
							$SERVER_API_KEY = 'AAAAXmNqxY4:APA91bG0X6RHVhwJKXUQGNSSCas44hruFdR6_CFd6WHPwx9abUr-WsrfEzsFInJawElgrp24QzaE4ksfmXu6kmIL6JG3yP487fierMys5byv-I1agRtMPIoSqdgCZf8R0iqsnds-u4CU';
							
							//define('SERVER_API_KEY',);
							$main_url=$app_url;
						}else if(!empty($web_token)){
							$device_tokens=$web_token;
							$SERVER_API_KEY = 'AIzaSyDQcVP0eF_55UrqTxYOAmdeeEzt2lQ6PAg';
							//define('SERVER_API_KEY',);	
							$main_url=$web_url;
						}
		...
		//if(($web_token!='NULL') || ($app_token!='NULL')){
					...
					if(!empty($web_token) || !empty($app_token)){
...
foreach($main_customer_ids as $main_custmr_data){
		...
		if(!empty($main_device_token1)){  
				...
				///////////////////////FOR//////IOS///////////////////////
							...
							$orderLink = $server_path.$main_url;			
							$url = 'https://fcm.googleapis.com/fcm/send';
							$body=["text" => $message,"link" =>$orderLink];
							
							$fcmRegIds = array();
							$fields = [
								"notification" => ["title" => $title, "text" => $message, "link" => $orderLink,"sound" => "default",'notification_id'=>$random,'message'=>'abc'],
...
////////////////////////////SECOND/////////////////////////////////					
		...
		$customers=$this->AppNotifications->AppNotificationCustomers->Customers->find('list')->where(['Customers.city_id'=>$city_id])->order(['name'=>'ASC']);
		
		$categories_data=$this->AppNotifications->Categories->find()->where(['Categories.status'=>'Active','Categories.city_id'=>$city_id]);
		
		foreach($categories_data as $cat_data){
			
			$categories[]=['text' =>$cat_data->name, 'value' => $cat_data->id,'parent_cat_id'=>$cat_data->parent_id];
		}
    ...
    public function view($ids = null)
    {
        ...
        $appNotification = $this->AppNotifications->get($id, [
            'contain' => ['Cities', 'Locations', 'ItemsView', 'ItemVariationsView', 'ComboOffersView', 'WishListsView', 'CategoriesView', 'AppNotificationCustomers']
        ]);
		//pr($appNotification);exit;
