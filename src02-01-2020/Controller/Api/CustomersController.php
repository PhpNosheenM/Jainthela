...
class CustomersController extends AppController
{
  public function initialize()
    {
        parent::initialize();
        $this->Auth->allow(['addAddress','add','push_token_update','login','send_otp','send_otpp','verify','setpassword','sendotpforgetpassword','editProfile','my_account','viewProfile','customerAddresses','removeAddress','']);
    }
			...
			if($exists_mobile==1){
				...
				if($this->Customers->VerifyOtps->save($VerifyOtps)){
					  ...
					  $query1 = $this->Customers->query();
						$query1->update()
								->set(['otp' => $opt])
								->where(['Customers.username'=>$mobile])
								->execute();
					 $success = true;
	...
	//	echo $exists_email = $this->Customers->exists(['Customers.email'=>$this->request->data['email']]); exit;
	 $customer = $this->Customers->newEntity();
		if($this->request->is(['patch', 'post', 'put'])){
				...
				$received_referral_code=@$this->request->data['referral_code'];						
				...
				if(($exists_email==0) and ($exists_mobile==0)){
					...
					//pr($customer); exit;
					 if ($customers=$this->Customers->save($customer)) {
						...
						if(!empty($received_referral_code))
						{
							$have_ref_code = $this->Customers->find()->where(['Customers.referral_code'=>$received_referral_code])
							->first();
							
							if(!empty($have_ref_code))
							{	
								$ref_customer_id = $have_ref_code->id;
								$promotions = $this->Customers->Promotions->newEntity();
								$promotions->city_id=$city_id;
								$promotions->offer_name='Referral Code';
								$promotions->description='Referral Code #'.$received_referral_code;
								$promotions->start_date=$today;
								$promotions->end_date=$end_date;
								$promotions->status = 'Deactive';
								$promo_data=$this->Customers->Promotions->save($promotions);
								$promo_id=$promo_data->id;	

								$jain_text='JT';
								$random=(string)mt_rand(01,99);
								$coupon_code=$jain_text.$ref_customer_id.$promo_id.$random;
								
								$promotion_details = $this->Customers->Promotions->PromotionDetails->newEntity();
								$promotion_details->promotion_id=$promo_id;
								$promotion_details->discount_in_percentage=2;
								...
								$promo_detail_data=$this->Customers->Promotions->PromotionDetails->save($promotion_details);

								$promo_detail_id=$promo_detail_data->id;
							
								$customer_promotion = $this->Customers->CustomerPromotions->newEntity();
								...
								$customer_promotion->customer_id=$ref_customer_id;
								$customer_promotion->city_id=$city_id;
								$customer_promotion->start_date=$today;
								$customer_promotion->end_date=$end_date; 
								$customer_promotion->status='Active'; 
								$this->Customers->CustomerPromotions->save($customer_promotion);
...
/*    public function editProfile($id=null)
       ...
       $isValidToken = $this->checkToken($token);
         ...
         if($isValidToken == 0)
           ...
           $customer = $this->Customers->find()
              ->contain(['Cities','CustomerAddresses'])->where(['Customers.id'=>$id]);
   ...
   public function customerAddresses($customer_id=null)
   {
       $customer_id = @$this->request->query['customer_id'];
     //  $token = @$this->request->query['token'];
       if(!empty($customer_id))
       {
         // checkToken function is avaliable in app controller for checking token in customer table
            ...
            ->contain(['Cities','Landmarks'])->where(['customer_id'=>$customer_id,'is_deleted'=>0]);
