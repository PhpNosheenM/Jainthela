								 ...
								 {$i=0;		
									...
									<tr class="MainTr" row_no="<?php echo $i;?>">
										<td width="10%">
											...
											{
												$options['Cr'] = 'Cr'; ?>
												<?= $this->Form->select('receipt_rows.'.$i.'.cr_dr',$options,['class'=>'form-control input-sm cr_dr select','label'=>false,'value'=>$receiptRows->cr_dr]) ?>
											<?php }
											else{
												 $options['Dr'] = 'Dr'; 
												 $options['Cr'] = 'Cr'; ?>
										...
										if($i==0)
										{ 
											echo $this->Form->select('receipt_rows.'.$i.'.ledger_id',$ledgerOptions, ['empty'=>'--Select--','label' => false,'class' => 'form-control input-sm ledger select','required'=>'required', 'data-live-search'=>true,'value'=>$receiptRows->ledger_id]); ?>
												...
												?>
													<tr>
														<td width="20%">
															...
															<?php 
															echo $this->Form->input('receipt_rows.'.$i.'.reference_details.'.$j.'.city_id', ['type'=>'hidden','label' => false,'class' => 'form-control input-sm companyIdContainer','value'=>$reference_detail->city_id]); ?>
															 
															<?php
																echo $this->Form->select('receipt_rows.'.$i.'.reference_details.'.$j.'.type',$option_ref, ['label' => false,'class' => 'form-control input-sm refType select','required'=>'required','value'=>$reference_detail->type]); ?>
														</td>
														
														<td width="">
														<?php if($reference_detail->type=='New Ref' || $reference_detail->type=='Advance'){ 
														?>
															<?php echo $this->Form->input('receipt_rows.'.$i.'.reference_details.'.$j.'.ref_name', ['type'=>'text','label' => false,'class' => 'form-control input-sm ref_name','placeholder'=>'Reference Name','required'=>'required']); ?>
											...
											<table width='90%'>
												<tbody>
													<tr>
														<td width="30%">
															<?php 
															echo $this->Form->input('receipt_rows.'.$i.'.mode_of_payment', ['options'=>$option_mode,'label' => false,'class' => 'form-control input-sm paymentType select','required'=>'required','value'=>$receiptRows->mode_of_payment]); ?>
														</td>
										...
										<td width="10%">
											 <?php if($receiptRows->debit > 0){ ?>
											<?php echo $this->Form->input('receipt_rows.'.$i.'.debit', ['label' => false,'class' => 'form-control input-sm debitBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Debit','value'=>$receiptRows->debit, 'type'=>'text']); ?>
											 <?php }else{ ?>
											 <?php echo $this->Form->input('receipt_rows.'.$i.'.debit', ['label' => false,'class' => 'form-control input-sm debitBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Debit','value'=>'', 'type'=>'text','style'=>'display:none;']); ?>
											 <?php } ?>
										</td>
										<td width="10%">
											 <?php if($receiptRows->credit > 0){ ?>
											<?php echo $this->Form->input('receipt_rows.'.$i.'.credit', ['label' => false,'class' => 'form-control input-sm creditBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Credit','value'=>$receiptRows->credit, 'type'=>'text']); ?>
											  <?php }else{ ?>
												<?php echo $this->Form->input('receipt_rows.'.$i.'.credit', ['label' => false,'class' => 'form-control input-sm creditBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Credit','value'=>'', 'type'=>'text','style'=>'display:none;']); ?>
											  <?php } ?>
										</td>
