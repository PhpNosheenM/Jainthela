					 ...
					 foreach($carts_data_fetch->combo_offers_data->combo_offer_details as $combo_offer_detail){
						...
						->first();
						   ...
						   $cate_id = $comboItemDetails->item->category_id;	
								...
								$this->request->data['order_details'][$i]['quantity']= $combo_offer_detail->quantity * $carts_data_fetch->quantity;
						...
						$i++;					 
							...
							$this->request->data['order_details'][$i]['combo_offer_id']=$carts_data_fetch->combo_offer_id;
							$this->request->data['order_details'][$i]['quantity']=$carts_data_fetch->quantity * $carts_data_fetch->quantity;
...
//			pr($order);exit;
	...
	public function ItemWiseCancelOrder()
	{
		...
		if(is_array($order_detail_ids) && !empty($order_detail_ids))
		{
			...
			if($isCombo == true && $combo_offer_id > 0)
			{
				$query->update()
				->set(['is_item_cancel'=>'Yes'])
			...
			{				
				foreach($OrderDetails->order_details as $order_detail)
				{
					...
					$order_detail->discount_amount = $order_detail->memberShipDiscount_Amount;
						...
						if($isPromoCode == 'true')
						{
							...
							$taxAbleAmount_Total = 0.00;
							$promo_amount = 0.00;						
							if($promotionDetails->discount_in_percentage > 0)
							{
								if(!empty($promo_item_id) && $promo_item_id > 0)
								{
									if($item_id == $promo_item_id)
						...
						else if($promotionDetails->discount_in_amount > 0)
						{  
							if(!empty($promo_item_id) && $promo_item_id > 0)
							{
								if($item_id == $promo_item_id)
								{
									if($taxAbleAmount_Total > 0)
									{
				...
				$OrderDetails->discount_amount=$total_membershipDsicount + $total_promoDiscount;			
				$OrderDetails->total_gst = $total_GST;
				$OrderDetails->pay_amount	= $total_amount + $total_GST;

				$query = $this->Orders->query();
				$query->update()
				->set(['total_amount'=>$OrderDetails->total_amount,'discount_amount'=>$OrderDetails->discount_amount,
				'total_gst' =>$OrderDetails->total_gst,'pay_amount' =>$OrderDetails->pay_amount])
