<?php $this->set('title', 'Sales Return'); ?>
		...
		<?= $this->Form->create($saleReturn,['id'=>'jvalidate','class'=>'form-horizontal','type'=>'file']) ?>  
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title"><strong> Sales Return </strong></h3>
				</div>
				...
				<div class="panel-body">    
					<div class="row">
						...
						<div class="form-group">
								...
								<div class=""> 
									<?= $this->Form->control('narration',['class'=>'form-control','placeholder'=>'Narration','label'=>false,'rows'=>'6','value'=>@$order->narration]) ?>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="row">
						<div class="table-responsive">
							<table class="table table-bordered main_table">
								...
								foreach($order->invoice_rows as $sales_order_row){
											@$g++;
								?>
									<tr class="MainTr">
										<td  valign="top"><?php echo $g; ?></td>
										<td  valign="top" class="itemList"> 
											<?= $this->Form->select('item_variation_id',$items,['empty'=>'--select--','style'=>'','class'=>'form-control item1','label'=>false,'readonly','disabled' ,'value'=>$sales_order_row->item_variation_id]) ?>
											
										
											
											<?= $this->Form->control('item_variation_id',['type'=>'hidden','class'=>'form-control item','label'=>false,'readonly', 'value'=>$sales_order_row->item_variation_id]) ?>
										...
										<td valign="top">
											<?= $this->Form->control('amount',['type'=>'hidden','class'=>'form-control amount','label'=>false,'readonly', 'value'=>$sales_order_row->amount]) ?>
								...
								<tfoot>
									...
									<tr>
										...
										<td colspan="2" style="text-align:right;"><?= $this->Form->control('amount_before_tax',['class'=>'form-control total_taxable_value','label'=>false,'readonly','value'=>@$order->total_amount]) ?></td>
									</tr>
									...
									<tr>
										...
										<td colspan="2" style="text-align:right;"><?= $this->Form->control('amount_after_tax',['class'=>'form-control total_amt','label'=>false,'readonly','value'=>@$order->grand_total]) ?></td>
									</tr>
								</tfoot>
							</table>
						</div>
	...
	$js="var jvalidate = $('#jvalidate').validate({
		...
		function renameRows(){ 
			...
			$('.main_table tbody tr').each(function(){
				...
				$(this).find('.item ').attr({name:'sale_return_rows['+i+'][item_variation_id]',id:'sale_return_rows['+i+'][item_variation_id]'}).rules('add', 'required');
				...
				$(this).find('.amount ').attr({name:'sale_return_rows['+i+'][amount]',id:'sale_return_rows['+i+'][amount]'});
				$(this).find('.taxable_value ').attr({name:'sale_return_rows['+i+'][taxable_value]',id:'sale_return_rows['+i+'][taxable_value]'});
		...
		function calculation(){ 
			...
			$('.main_table tbody tr').each(function(){ 
					...
					var taxable_value=taxable_value-gst_rate;
					$(this).find('.taxable_value').val(taxable_value);
					
					var net_amount=taxable_value+gst_rate;
