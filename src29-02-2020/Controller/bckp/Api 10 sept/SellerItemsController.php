 ...
 public function initialize()
  {
	...
	public function popularItem()
	{
      ...
      if($isValidCity == 0)
      {
        ...
        $sellerItem = $this->SellerItems->find();
         $sellerItem-> contain(['Sellers','ItemRating' => function($q) use($sellerItem) {
				...
				},'Items','ItemVariations' => ['UnitVariations'=>['Units'],'ItemVariationMasters']])
		...
		->where(['SellerItems.item_id IN' =>$item_ids])
	  ...
	  $this->set(compact('success','message','cart_item_count','sellerItem'));
      $this->set('_serialize', ['success','message','cart_item_count','sellerItem']);
...
//			pr($items->toArray());exit;
      ...
      public function searchSuggestion($city_id=null,$item_name=null)
      {
        ...
        if(!empty($item_name))
        {
            ...
            if(!empty($items->toArray())){
              foreach ($items as $item) { 
                $ItemData[] = ['category_id' =>$item->item->category_id,'name'=>$item->item->name .' in '. $item->item->category->name,'image' => $item->item->item_image,'item_name'=>$item->item->name,'category_name'=>$item->item->category->name];
              }
            }
            $success = true;
            $message = 'Data found';
