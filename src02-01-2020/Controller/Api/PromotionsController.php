...
class PromotionsController extends AppController
{
	...
	{
		parent::initialize();
		$this->Auth->allow(['promocodeList','checkPromoCodes']);		
	}
	...
	public function promocodeList($city_id=null,$customer_id=null,$token=null)
	{
		...
		if($isValidToken == 0)
		{
			if(!empty($city_id))
			{
				...
				->contain(['Promotions'=>function($q) use($city_id,$conditions){
					return $q->contain(['PromotionDetails'])->where(['Promotions.city_id'=>$city_id])
				->where([$conditions]);		
				}])->where(['CustomerPromotions.customer_id' =>$customer_id,'Promotions.status'=>'Active']);
			...
			if(!empty($city_id))
			{
				if(!empty($coupon_code))
				{
					...
					->contain(['PromotionDetails' => function ($q) use($coupon_code) {
						...
						->where(['PromotionDetails.Status' =>'Active']);
					}])
					->where([$conditions])
					...
					{	
						foreach($promo_codes as $promo_code)
						{
							...
							{
								foreach($promo_code->promotion_details as $promotion_details)
								{
									$customerUsedCount = $this->Promotions->CustomerPromotions->find('All')
									->where(['CustomerPromotions.status' =>'Deactive','CustomerPromotions.customer_id'=>$customer_id,'promotion_detail_id'=>$promotion_details->id])->count();
									...
									{
										$promotion_details->original_promotion_id = $promotion_details->promotion_id;
										$promotion_details->promotion_id = $promotion_details->id;
										$promoData[] = $promotion_details;
										$success = true;
										$message = "Valid Code";									
									}									
								}
					...
					{	
						...
						if(!empty($promo_codes))
						{
							...
							{
								$customerUsedCount = $this->Promotions->CustomerPromotions->find('All')
								->where(['CustomerPromotions.status' =>'Deactive','CustomerPromotions.customer_id'=>$customer_id,'promotion_detail_id'=>$promo_codes->promotion_details[0]->id])->count();
								if($customerUsedCount > 0)
								{
									$success = false;
									$message = "Customer has already used code";									
								...
								{
									//pr($promo_codes->promotion_details);exit;
									$promo_codes->promotion_details[0]->original_promotion_id = $promo_codes->promotion_details[0]->promotion_id;
									$promo_codes->promotion_details[0]->promotion_id = $promo_codes->promotion_details[0]->id;
