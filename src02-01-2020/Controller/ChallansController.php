...
//////////////Notification//Code///Start//////////////////
	...
	//file_get_contents('http://103.39.134.40/api/mt/SendSMS?user=phppoetsit&password=9829041695&senderid='.$sms_sender.'&channel=Trans&DCS=0&flashsms=0&number='.$mob.'&text='.$sms.'&route=7');	
		...
		if(!empty($status)){
			$order_data=$this->Challans->find()->where(['Challans.order_status'=>$status])->order(['Challans.id'=>'DESC'])->contain(['Orders','SellerLedgers','PartyLedgers','Locations','Customers','CustomerAddressesLeft'=>['Landmarks'=>['RouteDetails'=>['Routes']]],'Drivers','ChallanRows'=>['Items'=>'Categories']])->where(['Challans.city_id'=>$city_id])->where($where_get);
		}else{
			//exit;
			$order_data=$this->Challans->find()->order(['Challans.id'=>'DESC'])->contain(['Orders','SellerLedgers','PartyLedgers','Locations','Customers','CustomerAddressesLeft'=>['Landmarks'=>['RouteDetails'=>['Routes']]],'Drivers','ChallanRows'=>['Items'=>'Categories']])->where(['Challans.city_id'=>$city_id,'Challans.order_status !='=>'Cancel'])->where(['Challans.order_status !='=>'Delivered'])->where($where_get);
		}
