...
class ItemsController extends AppController
{
    ...
    public function SelectItemVariation($id = null,$unit_variation_id = null){
			$Sellertem=$this->Items->ItemVariations->find()->contain(['Items','UnitVariations'=>['Units']])->where(['ItemVariations.item_id'=>$id,'ItemVariations.seller_id IS NULL','ready_to_sale'=>'Yes']);
			$items=array();
			foreach($Sellertem as $data){
				//pr(@$data); 
        ...
        if ($this->request->is(['patch', 'post', 'put'])) { 
			 ...
			 foreach($item as $data){ 
				...
				if($itemVarData->seller_id > 0){
					$sellerAllItem=$this->Items->ItemVariations->find()->where(['seller_id'=>$itemVarData->seller_id,'seller_item_id'=>$itemVarData->seller_item_id,'ready_to_sale'=>'Yes'])->first();
					if($sellerAllItem){
					...
					$sellerAllItem=$this->Items->ItemVariations->find()->where(['seller_id IS NULL','seller_item_id'=>$itemVarData->seller_item_id,'ready_to_sale'=>'Yes'])->first();
					if($sellerAllItem){
						$query1 = $this->Items->SellerItems->query();
						$query1->update()
							->set(['status' =>'Active'])
...
/* pr($stockNew); exit;
    ...
    { 
		...
		if(!empty($item_id))
		{
		 ...
		 $itemLedgers = $this->Items->ItemLedgers->find()->where($where)->order(['transaction_date'=>'DESC']);
