		...
		}else
		{
			...
			if(!empty($categories->toArray()))
			{
				...
				->contain(['ItemVariations'=> function($q){
					return $q->where(['out_of_stock'=>'No'])->contain(['ItemVariationMasters','Items' =>['GstFigures','Categories'] ,'UnitVariations'=>['Units']]);
				}])
				->group('Carts.item_variation_id')->autoFields(true)->toArray();
...
/* 												$item_id = $cart_data->item_variation->item->id;
		...
		$deliveryString = "Your order will be deliver in next 48 hours !";		
	...
	$OutofStock_comboData =[]; $OutofStock_carts_data =[];
			...
			->where(['Carts.city_id' =>$city_id])
			->contain(['ItemVariations'=> function($q){
				return $q->where(['out_of_stock'=>'Yes']);}])
			->group('Carts.item_variation_id')->autoFields(true)->toArray();		
