...
<?php $this->set('title', 'Sales Return Report'); ?><!-- PAGE CONTENT WRAPPER -->
		...
		<div class="col-md-12">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title"><strong>SALES RETURN REPORT</strong></h3>
						...
						<table class="table table-bordered">
							...
							<tbody>                                            
								...
								$i = 0; $total_sales_amount=0; $total_gst_amount=0; $total_amount=0;?>
								  <?php foreach ($orders as $order): $order->BgColor = '';
									
									?>
								
								<tr height="40px"  >
									<td <?php echo $order->BgColor; ?>><?= $this->Number->format(++$i) ?></td>
									<td <?php echo $order->BgColor; ?>><?= h($order->voucher_no) ?></td>
									<td <?php echo $order->BgColor; ?>>
									<?php $order_id = $EncryptingDecrypting->encryptData($order->invoice->id); ?>
											<?php echo $this->Html->link($order->invoice->invoice_no,['controller'=>'Invoices','action' => 'pdfView', $order_id, 'print'],['target'=>'_blank']); ?>
									<?php //echo $order->invoice->invoice_no;?>
									</td>
									<td <?php echo $order->BgColor; ?>><?= h($order->party_ledger->name) ?></td>
									<td  <?php echo $order->BgColor; ?>><?= h(@$order->party_ledger->customer_data->gstin) ?></td>
									<td <?php echo $order->BgColor; ?>><?= h(date("d-m-Y",strtotime($order->transaction_date))) ?></td>
									<td <?php echo $order->BgColor; ?>><?php echo $this->Money->moneyFormatIndia($order->amount_before_tax,2); ?></td>
									<td <?php echo $order->BgColor; ?>><?php echo $this->Money->moneyFormatIndia($order->amount_after_tax-$order->amount_before_tax,2); ?></td>
									<td <?php echo $order->BgColor; ?>><?php echo $this->Money->moneyFormatIndia($order->amount_after_tax,2); ?></td>
									<?php
