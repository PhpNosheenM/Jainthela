		...
		function renameMainRows(){
			...
			function renameBankRows(SelectedTr){
				...
				SelectedTr.find('td:nth-child(2) div.window table tbody tr').each(function(){
					...
					{
						$(this).find('td:nth-child(2) input.cheque_no').attr({ name:'q'}).rules('remove','required');
						$(this).find('td:nth-child(3) input.cheque_date').attr({ name:'q'}).rules('remove','required');
											}
				});
			}
