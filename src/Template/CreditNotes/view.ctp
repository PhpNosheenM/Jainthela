...
<div  class="maindiv" style="border:solid 1px #c7c7c7;background-color: #FFF;padding: 10px;margin: auto;width:75%;font-size: 12px;">	
	<table width="100%" class="divHeader">
		...
		Narration: <?php echo $creditNote->narration;?>
		<br/><br/>
		<table width="100%" class="table" style="font-size:12px">
			...
			<?php foreach($creditNote->credit_note_rows as $credit_note_row)
				{ 
					@$total_debit+=$credit_note_row->debit;
					@$total_credit+=$credit_note_row->credit; ?>
					<tr>
					<td colspan="3" style="text-align:left"><b><?= $credit_note_row->ledger->name ?>
					
					<?php if($credit_note_row->mode_of_payment=='Cheque'){  
						echo '( Cheque no-'.$credit_note_row->cheque_no.', Date '.date('d-M-Y', strtotime($credit_note_row->cheque_date)).')';
					} ?>
					</b>
