...
<div class="page-content-wrap">
	<div class="row">
		<div class="col-md-12">
		<?= $this->Form->create($contraVoucher,['id'=>'jvalidate','class'=>'form-horizontal','type'=>'file']) ?>  
							...
							<table id="MainTable" class="table table-condensed table-striped" width="100%">
							  ...
							  //  unset($option_ref);								 
								...
								$option_mode[]= ['value'=>'NEFT/RTGS','text'=>'NEFT/RTGS'];
								 ...
								 {$i=0;		
								         foreach($contraVoucher->contra_voucher_rows as $contraVoucherRows)
									     { ?>
									
									<tr class="MainTr" row_no="<?php echo $i;?>">
										<td width="10%">
											<?php 
											echo $this->Form->input('contra_voucher_rows.'.$i.'.id',['value'=>$contraVoucherRows->id,'class'=>'hidden']);
											if($i==0)
											{
												$options['Dr'] = 'Dr'; ?>
												<?= $this->Form->select('contra_voucher_rows.'.$i.'.cr_dr',$options,['class'=>'form-control input-sm cr_dr select','label'=>false,'value'=>$contraVoucherRows->cr_dr]) ?>
											<?php }
											else{
												 $options['Dr'] = 'Dr'; 
												 $options['Cr'] = 'Cr'; ?>
										...
										<td width="65%">
										<input type="hidden" class="BankValueDefine" name=" contra_voucher_rows[<?php echo $i;?>][BankDefination]">
							
										<?php
										if($i==0)
										{ 
											echo $this->Form->select('contra_voucher_rows.'.$i.'.ledger_id',$ledgerOptions, ['empty'=>'--Select--','label' => false,'class' => 'form-control input-sm ledger select','required'=>'required', 'data-live-search'=>true,'value'=>$contraVoucherRows->ledger_id]); ?>
										...
										?>
											...
											<?php
											if(!empty($contraVoucherRows->reference_details)){
											?>
												<table width="90%" class="refTbl"><tbody>
												<?php
												    $j=0;$total_amount_dr=0;$total_amount_cr=0;$colspan=0; 
												    foreach($contraVoucherRows->reference_details as $reference_detail)
													{
												?>
													<tr>
														<td width="20%">
															<?php 
															echo $this->Form->input('contra_voucher_rows.'.$i.'.reference_details.'.$j.'.ledger_id', ['type'=>'hidden','label' => false,'class' => 'form-control input-sm ledgerIdContainer','value'=>$reference_detail->ledger_id]); ?>
															
															
															
															<?php 
															echo $this->Form->input('contra_voucher_rows.'.$i.'.reference_details.'.$j.'.city_id', ['type'=>'hidden','label' => false,'class' => 'form-control input-sm companyIdContainer','value'=>$reference_detail->city_id]); ?>
															 
															<?php
																echo $this->Form->select('contra_voucher_rows.'.$i.'.reference_details.'.$j.'.type',$option_ref, ['label' => false,'class' => 'form-control input-sm refType select','required'=>'required','value'=>$reference_detail->type]); ?>
														</td>
														
														<td width="">
														<?php if($reference_detail->type=='New Ref' || $reference_detail->type=='Advance'){ 
														?>
															<?php echo $this->Form->input('contra_voucher_rows.'.$i.'.reference_details.'.$j.'.ref_name', ['type'=>'text','label' => false,'class' => 'form-control input-sm ref_name','placeholder'=>'Reference Name','required'=>'required']); ?>
															<?php } if($reference_detail->type=='Against')
															{?>
															<?php 
															if(!empty($refDropDown[$contraVoucherRows->id]))
															{
																echo $this->Form->input('contra_voucher_rows.'.$i.'.reference_details.'.$j.'.ref_name', ['options'=>@$refDropDown[$contraVoucherRows->id],'label' => false,'class' => 'form-control input-sm paymentType refList','required'=>'required','value'=>$reference_detail->ref_name]);
														...
														<td width="20%" style="padding-right:0px;" valign="top">
															...
															echo $this->Form->input('contra_voucher_rows.'.$i.'.reference_details.'.$j.'.'.$name, ['label' => false,'class' => 'form-control input-sm calculation numberOnly rightAligntextClass','placeholder'=>'Amount','required'=>'required','value'=>$value, 'type'=>'text']); ?>
														</td>
												...
												<tfoot>
												    <tr class="remove_ref_foot">
														...
														<td valign="top">
														<input type="text" class="form-control input-sm rightAligntextClass total calculation noBorder" name="contra_voucher_rows[<?php echo $i;?>][total]" id="contra_voucher_rows-<?php echo $i;?>-total" aria-invalid="true" aria-describedby="contra_voucher_rows-<?php echo $i;?>-total-error" value="<?php echo $total;?>" readonly>
														</td>
														<td valign="top"><input type="text" class="form-control input-sm total_type calculation noBorder" readonly value="<?php echo @$type;?>" name="contra_voucher_rows<?php echo $i;?>reference_details<?php echo $i;?>type_cr_dr"></td>
													</tr>
												</tfoot>
												</table>
											...
											<?php } ?>
											<?php
											if(!empty($contraVoucherRows->mode_of_payment)){
											?>
											<table width='90%'>
												<tbody>
													<tr>
														<td width="30%">
															<?php 
															echo $this->Form->input('contra_voucher_rows.'.$i.'.mode_of_payment', ['options'=>$option_mode,'label' => false,'class' => 'form-control input-sm paymentType select','required'=>'required','value'=>$contraVoucherRows->mode_of_payment]); ?>
														</td>
														
													<?php if($contraVoucherRows->mode_of_payment=='NEFT/RTGS'){?>
														 <?php $style='display:none';?>
														<?php } else if($contraVoucherRows->mode_of_payment=='Cheque'){ ?>
														 <?php $style='';?>
														
														
														
														<td width="30%" style="<?php echo $style;?>">
															<?php echo $this->Form->input('contra_voucher_rows.'.$i.'.cheque_no', ['label' =>false,'class' => 'form-control input-sm cheque_no','placeholder'=>'Cheque No','value'=>$contraVoucherRows->cheque_no]); ?>
														<?php } ?>		
														</td>
														<td width="30%" style="<?php echo $style;?>">
															<?php echo $this->Form->input('contra_voucher_rows.'.$i.'.cheque_date', ['label' =>false,'class' => 'form-control input-sm date-picker cheque_date ','data-date-format'=>'dd-mm-yyyy','placeholder'=>'Cheque Date','value'=>date("d-m-Y",strtotime($contraVoucherRows->cheque_date)),'type'=>'text']); ?>
														</td>
													</tr>
												</tbody>
												<tfoot>
										...
										<td width="10%">
											 <?php if($contraVoucherRows->debit > 0){ ?>
											<?php echo $this->Form->input('contra_voucher_rows.'.$i.'.debit', ['label' => false,'class' => 'form-control input-sm debitBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Debit','value'=>$contraVoucherRows->debit, 'type'=>'text']); ?>
											 <?php }else{ ?>
											 <?php echo $this->Form->input('contra_voucher_rows.'.$i.'.debit', ['label' => false,'class' => 'form-control input-sm debitBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Debit','value'=>'', 'type'=>'text','style'=>'display:none;']); ?>
											 <?php } ?>
										</td>
										<td width="10%">
											 <?php if($contraVoucherRows->credit > 0){ ?>
											<?php echo $this->Form->input('contra_voucher_rows.'.$i.'.credit', ['label' => false,'class' => 'form-control input-sm creditBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Credit','value'=>$contraVoucherRows->credit, 'type'=>'text']); ?>
											  <?php }else{ ?>
												<?php echo $this->Form->input('contra_voucher_rows.'.$i.'.credit', ['label' => false,'class' => 'form-control input-sm creditBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Credit','value'=>'', 'type'=>'text','style'=>'display:none;']); ?>
											  <?php } ?>
										</td>
	 ...
	 $js="var form1 = $('#jvalidate').validate({
		...
		function renameMainRows(){
				...
				$('#MainTable tbody#MainTbody tr.MainTr').each(function(){
					...
					$(this).find('td:nth-child(1) select.cr_dr').attr({name:'contra_voucher_rows['+i+'][cr_dr]',id:'contra_voucher_rows-'+i+'-cr_dr'}).selectpicker();
					$(this).find('td:nth-child(2) select.ledger').attr({name:'contra_voucher_rows['+i+'][ledger_id]',id:'contra_voucher_rows-'+i+'-ledger_id'}).selectpicker();
			...
			function renameBankRows(SelectedTr){
				...
				SelectedTr.find('td:nth-child(2) div.window table tbody tr').each(function(){
					...
					//alert(type);
					$(this).find('td:nth-child(1) select.paymentType').attr({name:'contra_voucher_rows['+row_no+'][mode_of_payment]',id:'contra_voucher_rows-'+row_no+'-mode_of_payment'}).selectpicker();
			...
			function renameRefRows(SelectedTr){
				...
				SelectedTr.find('td:nth-child(2) div.window table tbody tr').each(function(){
					$(this).find('td:nth-child(1) input.companyIdContainer').attr({name:'contra_voucher_rows['+row_no+'][reference_details]['+i+'][location_id]',id:'contra_voucher_rows-'+row_no+'-reference_details-'+i+'-location_id'});
					$(this).find('td:nth-child(1) input.ledgerIdContainer').attr({name:'contra_voucher_rows['+row_no+'][reference_details]['+i+'][ledger_id]',id:'contra_voucher_rows-'+row_no+'-reference_details-'+i+'-ledger_id'});
					
					$(this).find('td:nth-child(1) select.refType').attr({name:'contra_voucher_rows['+row_no+'][reference_details]['+i+'][type]',id:'contra_voucher_rows-'+row_no+'-reference_details-'+i+'-type'}).selectpicker();
					var is_select=$(this).find('td:nth-child(2) select.refList').length;
					var is_input=$(this).find('td:nth-child(2) input.ref_name').length;
					if(is_select){
						$(this).find('td:nth-child(2) select.refList').attr({name:'contra_voucher_rows['+row_no+'][reference_details]['+i+'][ref_name]',id:'contra_voucher_rows-'+row_no+'-reference_details-'+i+'-ref_name'}).rules('add', 'required');
						$(this).find('td:nth-child(2) select.refList').selectpicker();
					}else if(is_input){
						$(this).find('td:nth-child(2) input.ref_name').attr({name:'contra_voucher_rows['+row_no+'][reference_details]['+i+'][ref_name]',id:'contra_voucher_rows-'+row_no+'-reference_details-'+i+'-ref_name'}).rules('add', 'required');
						 
					}
					$(this).find('td:nth-child(4) select.refDrCr').selectpicker();
					var Dr_Cr=$(this).find('td:nth-child(4) select option:selected').val();
					if(Dr_Cr=='Dr'){
						$(this).find('td:nth-child(3) input').attr({name:'contra_voucher_rows['+row_no+'][reference_details]['+i+'][debit]',id:'contra_voucher_rows-'+row_no+'-reference_details-'+i+'-debit'}).rules('add', 'required');
					}else{
						$(this).find('td:nth-child(3) input').attr({name:'contra_voucher_rows['+row_no+'][reference_details]['+i+'][credit]',id:'contra_voucher_rows-'+row_no+'-reference_details-'+i+'-credit'}).rules('add', 'required');
					}
					i++;
				});
				...
				.attr({name:'contra_voucher_rows['+row_no+'][total]',id:'contra_voucher_rows-'+row_no+'-total'})
						.rules('add', {
							equalTo: '#'+eqlClass,
							messages: {
