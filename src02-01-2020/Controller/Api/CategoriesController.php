	 ...
	 public function initialize()
    {
	 ...
	 public function category(){
		...
		if(!empty($category_id) and (!empty($city_id))){
			...
			if($isValidCity == 0)
			{
				$data = $this->Categories->find()->where(['id'=>$category_id,'city_id'=>$city_id,'section_show'=>'Yes','status'=>'Active'])->order(['Categories.name'=>'ASC'])->contain(['ChildCategories'=> function($q) {
					return $q->order(['ChildCategories.name'=>'ASC']);
				}]);
