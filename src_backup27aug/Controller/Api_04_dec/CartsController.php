...
class CartsController extends AppController
{
	...
	public function initialize()
	 {
			 parent::initialize();
			 $this->Auth->allow(['getCount','updateAmount','fetchCart','plusAddToCart','removeFromCart','reviewOrder']);
	 }
		...
		if($exists == 1)
		{
				...
				if(!empty($carts_data_update))
				{
					...
					else
					{
					  $success = true;
					  $message = 'Cart Data found'; 
					  $this->set(compact('success', 'message','isPromoValidForAll','isPromoValidForAllMsg','Round_off_amt','address_available','delivery_charge_amount','payableAmount','remaining_wallet_amount','carts','item_in_cart','comboData','Combototal','promo_detail_id','ItemWiseTotal_taxable','ItemWiseTotal_With_GST','Combototal_with_GST','TotalBeforeDiscount','Sub_Total','PromoTotal','MembershipTotal','Total_GST','delivery_charge_message'));
					  
					  $this->set('_serialize', ['success','message','delivery_charge_message','isPromoValidForAll','isPromoValidForAllMsg','promo_detail_id','remaining_wallet_amount','item_in_cart','address_available','carts','ItemWiseTotal_taxable','ItemWiseTotal_With_GST','comboData','Combototal','Combototal_with_GST','TotalBeforeDiscount','Sub_Total','PromoTotal','MembershipTotal','Total_GST','delivery_charge_amount','Round_off_amt','payableAmount']);
					}
		...
		}else
		{
			...
			if(!empty($categories->toArray()))
			{
				...
				->contain(['ItemVariations'=> function($q){
					return $q->where(['out_of_stock'=>'No'])->contain(['Sellers','ItemVariationMasters','Items' =>['GstFigures','Categories'] ,'UnitVariations'=>['Units']]);
				}])
				->group('Carts.item_variation_id')->autoFields(true)->toArray();
