								...
								<div class="col-md-9 col-xs-12">
									...
									foreach ($unit_option as $data) {
										...
										echo '<div class="checkbox">';
											echo '<label>';
											if(!empty($VariationMasterArr[$data['value']]))
											{
												$chk ='checked';$disabled="";
											}else{$chk ='';$disabled="disabled";}
										 		echo $this->Form->checkbox('item_variation_row['.$i.'][unit_variation_id]',['value'=>$data['value'], 'hiddenField' => false,'label' => false,'class'=>'chk_class','id'=>'class'.$i,'divid'=>'deleteDisabled'.$i,$chk]);
										 		echo $data['text']; 
										 	echo '</label>';
										 	?>
											<div id="web_image_data<?php echo $i;?>" class="deleteDisabled<?php echo $i;?>">
											<?= $this->Form->control('item_variation_row['.$i.'][item_image_web]',['type'=>'file','label'=>false,'id' => 'item_image_'.$i,'data-show-upload'=>false, 'data-show-caption'=>false,@$disabled, @$required,'class'=>'class'.$i]) ?>
											<?php $required=true; 
											if(!empty($img[$data['value']]))
											{ 
												$keyname = $img[$data['value']];
												if(!empty($keyname))
												{
											...
											$js.=' $( document ).ready(function() {
														...
														$("#web_image_data'.$i.'").find("div.file-preview-frame").addClass("file-preview-frame krajee-default  kv-preview-thumb");
													
														$("#web_image_data'.$i.'").find("img").addClass("file-preview-image kv-preview-data rotate-1");
													});
											';
											}
											}
