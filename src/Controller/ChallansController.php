...
//	pr($order_id);exit;
	...
	public function manageOrderExcel($status=null)
    {
		$urll=$this->request->here();
		$urll=parse_url($urll,PHP_URL_QUERY);
		$user_id=$this->Auth->User('id');
		$city_id=$this->Auth->User('city_id');
		$location_id=$this->Auth->User('location_id');
		$status = $this->request->query('status');
		...
		$CityData = $this->Challans->Cities->get($city_id);
		$StateData = $this->Challans->Cities->States->get($CityData->state_id);	$where=[];
		if($status=="delivered" || $status=="cancel" || $status=="return"){
			$where['Challans.order_status']='%'.$status.'%';
		}else{
			$status='';
		...
		if(!empty($status)){
			$order_data=$this->Challans->find()->where(['Challans.order_status'=>$status])->order(['Challans.voucher_no'=>'DESC'])->contain(['Orders','SellerLedgers','PartyLedgers','Locations','Customers','CustomerAddressesLeft'=>['Landmarks'],'Drivers','ChallanRows'=>['Items'=>'Categories']])->where(['Challans.city_id'=>$city_id])->where($where_get);
		}else{
			//exit;
			$order_data=$this->Challans->find()->order(['Challans.voucher_no'=>'DESC'])->contain(['Orders','SellerLedgers','PartyLedgers','Locations','Customers','CustomerAddressesLeft'=>['Landmarks'],'Drivers','ChallanRows'=>['Items'=>'Categories']])->where(['Challans.city_id'=>$city_id,'Challans.order_status !='=>'Cancel'])->where(['Challans.order_status !='=>'Delivered'])->where($where_get);
		}
		...
		//pr($status); exit;
		$paginate_limit=$this->paginate['limit'];
        $this->set(compact('orders','paginate_limit','status','status1','drivers','cancelReasons','AllCategories','TotalChallans','Routes','RouteWiseChallan','ChallanWiseRoute','from_date','to_date','user_type','urll'));
    }
	...
	public function manageOrder($status=null)
    {
		...
		if ($this->request->is('post')) { 
			...
			$Invoice->cod_amount=@$Challan->take_cod_amount; 
			...
			if($InvoiceData->cod_amount > 0){
				$InvoiceData->pay_amount=$InvoiceData->pay_amount-$InvoiceData->cod_amount;
			}
			...
			if($InvoiceData->order_type ==  "Wallet"){
				...
				if($InvoiceData->cod_amount > 0){
					...
					$AccountingEntrie->debit=$InvoiceData->cod_amount;
					$AccountingEntrie->credit=0;
					$AccountingEntrie->transaction_date=date('Y-m-d');
					$AccountingEntrie->city_id=$InvoiceData->city_id;
					$AccountingEntrie->entry_from="Web";
				...
				if($InvoiceData->cod_amount > 0){
					...
					$AccountingEntrie = $this->Challans->Invoices->Ledgers->AccountingEntries->newEntity(); 
					$AccountingEntrie->ledger_id=$cash_acc_ledger_id;
					$AccountingEntrie->debit=$InvoiceData->pay_amount+$InvoiceData->cod_amount;
					$AccountingEntrie->credit=0;
					$AccountingEntrie->transaction_date=date('Y-m-d');
					$AccountingEntrie->city_id=$InvoiceData->city_id;
					$AccountingEntrie->entry_from="Web";
...
//////////////Notification//Code///Start//////////////////
	...
	//file_get_contents('http://103.39.134.40/api/mt/SendSMS?user=phppoetsit&password=9829041695&senderid='.$sms_sender.'&channel=Trans&DCS=0&flashsms=0&number='.$mob.'&text='.$sms.'&route=7');	
		...
		if($status=="delivered" || $status=="cancel" || $status=="Returned"){
			$where['Challans.order_status']='%'.$status.'%';
		}else{
			$status='';
		}
		...
		if(!empty($status)){
			$order_data=$this->Challans->find()->where(['Challans.order_status'=>$status])->order(['Challans.voucher_no'=>'DESC'])->contain(['Orders','SellerLedgers','PartyLedgers','Locations','Customers','CustomerAddressesLeft'=>['Landmarks'=>['RouteDetails'=>['Routes']]],'Drivers','ChallanRows'=>['Items'=>'Categories']])->where(['Challans.city_id'=>$city_id])->where($where_get);
		}else{
			//exit;
			$order_data=$this->Challans->find()->order(['Challans.voucher_no'=>'DESC'])->contain(['Orders','SellerLedgers','PartyLedgers','Locations','Customers','CustomerAddressesLeft'=>['Landmarks'=>['RouteDetails'=>['Routes']]],'Drivers','ChallanRows'=>['Items'=>'Categories']])->where(['Challans.city_id'=>$city_id,'Challans.order_status !='=>'Cancel'])->where(['Challans.order_status !='=>'Delivered'])->where(['Challans.order_status !='=>'Returned'])->where($where_get);
		}
	 ...
	 public function challanView($id = null)
    {
		...
		$sales_orders=$this->Challans->find()->where(['Challans.id'=>$ids])->contain(['Orders','Drivers', 'Customers'=>['CustomerAddresses'=>'Landmarks'],'ChallanRows'=> function ($q) {
		...
		$ordercount=$this->Challans->find()->where(['Challans.order_id'=>$order_id])->count();
		$this->loadmodel('SalesOrders');
		$challan=$this->Challans->find()->where(['Challans.id'=>$ids])->contain(['Orders','Drivers','Customers'=>['CustomerAddresses'],'CustomerAddresses'=>['Landmarks'],'ChallanRows'=>
		function($q) {
	...
	//	 pr($challan); exit;
		...
		$company_details=$this->Challans->Companies->find()->where(['Companies.city_id'=>$city_id])->first();
        $this->set(compact('ids', 'challan', 'order', 'locations', 'customers', 'drivers', 'customerAddresses', 'promotionDetails', 'deliveryCharges', 'deliveryTimes', 'cancelReasons','order_no','partyOptions','Accountledgers','items','company_details','ordercount'));
    }
