...
class InvoicesController extends AppController
{
    ...
    public function index()
    {
		...
		if($status=="return"){
			$invoices = $this->paginate($this->Invoices->find()->where($where_get)->where(['Invoices.seller_id IS NULL','Invoices.sales_return_status'=>'Pending','Invoices.invoice_status'=>'Done'])->order(['Invoices.id' => 'ASC']));
		}else{
     ...
     */
	 public function itemLedgerUpdate(){
		$Invoices=$this->Invoices->find()->where(['Invoices.invoice_status'=>'Done','seller_id IS NULL'])->contain(['InvoiceRows'=>['ItemVariations'=>['UnitVariations']]]);
	...
	// pr($ids); exit;
		$this->loadmodel('SalesOrders');
		$Orders=$this->Invoices->get($ids,['contain'=>['Cities','InvoiceRows'=>['ItemVariations'=>['UnitVariations'],'Items'],'Customers'=>['CustomerAddresses','Cities'],'CustomerAddresses']]);
