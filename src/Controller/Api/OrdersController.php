...
/*                 $delivery_charges=$this->Orders->DeliveryCharges->find()->where(['city_id'=>$city_id]);
    ...
    {	
      ...
      if($this->request->is('post')) {
		  ...
		  $system_setting=$this->settings->get(1);
		  
		  $delivery_date = date('Y-m-d',strtotime("+".$system_setting->delivery_day." days"));
		  $isPromoCode ='';
					 ...
					 foreach($carts_data_fetch->combo_offers_data->combo_offer_details as $combo_offer_detail){
						...
						->first();
						   ...
						   $quantity = $carts_data_fetch->quantity * $combo_offer_detail->quantity;	
								...
								$this->request->data['order_details'][$i]['wallet_amount']=$combo_offer_detail->amount;									
						...
						$i++;					 
							...
							$this->request->data['order_details'][$i]['wallet_amount']=$amount;		
