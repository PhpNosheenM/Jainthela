...
class CitiesController extends AppController
{
	...
	public function masterSetup($ids = null)
    {
		$user_id=$this->Auth->User('id');
		...
		$masterSetups = $this->Cities->MasterSetups->find()->where(['MasterSetups.city_id'=>$city_id]);
        if($ids)
		{
		    $masterSetup = $this->Cities->MasterSetups->get($id);
		}
		else
		{
			 $masterSetup = $this->Cities->MasterSetups->newEntity();
		}

        if ($this->request->is(['post','put'])) {
            $masterSetup = $this->Cities->MasterSetups->patchEntity($masterSetup, $this->request->getData());
			$masterSetup->city_id=$city_id;
			if ($this->Cities->MasterSetups->save($masterSetup)) {
				$this->Flash->success(__('The Master Setup has been Added.'));
			} else {
				$this->Flash->error(__('The  Master Setup has been Deleted.'));
			}
			return $this->redirect(['action' => 'masterSetup']);
			
		}	
	
	
		$masterSetups = $this->paginate($masterSetups);
		$paginate_limit=$this->paginate['limit'];
        $this->set(compact('masterSetups','masterSetup','paginate_limit'));
    }
