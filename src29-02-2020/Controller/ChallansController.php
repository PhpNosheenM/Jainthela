...
//	pr($order_id);exit;
	...
	public function manageOrderExcel($status=null)
    {
		...
		if(!empty($status)){
			$order_data=$this->Challans->find()->where(['Challans.order_status'=>$status])->order(['Challans.voucher_no'=>'DESC'])->contain(['Orders','SellerLedgers','PartyLedgers','Locations','Customers','CustomerAddressesLeft','Drivers','ChallanRows'=>['Items'=>'Categories']])->where(['Challans.city_id'=>$city_id])->where($where_get);
		}else{
			//exit;
			$order_data=$this->Challans->find()->order(['Challans.voucher_no'=>'DESC'])->contain(['Orders','SellerLedgers','PartyLedgers','Locations','Customers','CustomerAddressesLeft','Drivers','ChallanRows'=>['Items'=>'Categories']])->where(['Challans.city_id'=>$city_id,'Challans.order_status !='=>'Cancel'])->where(['Challans.order_status !='=>'Delivered'])->where($where_get);
		}
