...
class AccountingEntriesController extends AppController
{
	...
	public function AccountStatement()
    {
		...
		//pr($Ledger); exit;
			 $AccountingLedgers=$this->AccountingEntries->find()->select(['total_credit_sum'=>'SUM(AccountingEntries.credit)','total_debit_sum'=>'SUM(AccountingEntries.debit)'])->contain(['Ledgers','PurchaseInvoices','Payments','Orders','Receipts','Invoices'])->where($where)->group(['AccountingEntries.payment_id','AccountingEntries.purchase_invoice_id','AccountingEntries.receipt_id','AccountingEntries.order_id','AccountingEntries.invoice_id'])->autoFields(true); 
			}
			...
			$this->set(compact('from_date','to_date', 'groupForPrint', 'closingValue', 'openingValue','Ledger','AccountingLedgers','ledger_id','opening_balance','opening_balance_type'));
	}
