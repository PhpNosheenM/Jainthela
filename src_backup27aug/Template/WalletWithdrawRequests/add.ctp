...
<?php $this->set('title', 'Wallet Refund'); ?>
<!-- PAGE CONTENT WRAPPER -->
<div class="page-content-wrap">
	<div class="row">
		<div class="col-md-12">
		<?= $this->Form->create($walletWithdrawRequest,['id'=>'jvalidate','class'=>'form-horizontal','type'=>'file']) ?>  
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title"><strong> Wallet Refund</strong></h3>
				</div>
					...
					<div class="row">
						<div class="">
							<table id="MainTable" class="table table-condensed table-striped" width="100%">
								...
								<?php foreach($SelectedCustomer as $key=>$data){ ?>
									<tr class="MainTr">
										...
										<td width="65%" valign="top">
											<?php echo $this->Form->input('wallet_withdraw_request_id', ['type'=>'hidden','class'=>'wallet_withdraw_request_id','value'=>@$$data['value']]); ?>
											<?php echo $this->Form->select('ledger_id_data',$customerLedgers, ['empty'=>'--Select--','label' => false,'class' => 'form-control input-sm ledger select','required'=>'required', 'data-live-search'=>true,'value'=>$data]); ?>
											
										</td>
										<td width="10%" valign="top">
											<?php echo $this->Form->input('debit', ['label' => false,'class' => 'form-control input-sm  debitBox rightAligntextClass numberOnly calculate_total','placeholder'=>'Debit','value'=>$RefundAmount[$key],'readonly'=>true]); ?>
										</td>
										<td width="10%" valign="top">
...
<table id="sampleMainTable" style="display:none;" width="100%">
	<tbody class="sampleMainTbody">
		<tr class="MainTr">
			...
			<td width="65%" valign="top">
				<?php echo $this->Form->select('ledger_id',$cashBankLedgers, ['empty'=>'--Select--','label' => false,'class' => 'form-control input-sm ledger ','required'=>'required', 'data-live-search'=>true]); ?>
	 ...
	 $js="var form1 = $('#jvalidate').validate({
		...
		function renameMainRows(){
				...
				$('#MainTable tbody#MainTbody tr.MainTr').each(function(){
					...
					$(this).find('.wallet_withdraw_request_id').attr({name:'payment_rows['+i+'][wallet_withdraw_request_id]',id:'payment_rows-'+i+'-wallet_withdraw_request_id'});
