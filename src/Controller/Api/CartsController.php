   ...
   public function removeFromCart()
   {
	...
	public function fetchCart()
    {
		...
		else if($tag=='minus')
		{
			...
			$query = $this->Carts->query();
			$result = $query->delete()
				->where(['item_variation_id' => $item_variation_id, 'customer_id' => $customer_id])
				->execute();
		...
		$carts=$this->Carts->find()
				...
				->select(['total'=>'sum(Carts.cart_count * ItemVariations.sales_rate)'])
				->group('Carts.item_variation_id')->autoFields(true);
				
				if($carts==null || empty($carts->toArray()))
				{
				$carts=[];
				}
				else{
		...
		//$delivery_charges=$this->DeliveryCharges->find();
		 ...
		 if($carts == null || empty($carts->toArray()))
		 {
			...
			$message = 'Cart Data found';
				$this->set(compact('success', 'message','address_available','grand_total', 'remaining_wallet_amount', 'carts', 'delivery_charges'));
				$this->set('_serialize', ['success', 'message','address_available','grand_total', 'remaining_wallet_amount',  'carts', 'delivery_charges']);
    	 }
	}	 
