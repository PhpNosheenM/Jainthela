...
class ContraVouchersController extends AppController
{
    ...
    public function add()
    {
		...
		if ($this->request->is('post')) {
			...
			$Voucher = $this->ContraVouchers->find()->select(['voucher_no'])->where(['city_id'=>$city_id,'financial_year_id'=>$financial_year_id])->order(['voucher_no' => 'DESC'])->first();
			if($Voucher)
			{
			...
			$contraVoucher->financial_year_id = $financial_year_id;
	...
	//pr($partyGroups->toArray()); exit;
		...
		foreach($contraVoucher->contra_voucher_rows as $contra_voucher_row)
		{
			if(!empty($contra_voucher_row->reference_details))
			{
				foreach($contra_voucher_row->reference_details as $referenceDetailRows)
				{
					@$ref_details_name[]=$referenceDetailRows->ref_name;
				}
				$query = $this->ContraVouchers->ContraVoucherRows->ReferenceDetails->find();
				$query->select(['total_debit' => $query->func()->sum('ReferenceDetails.debit'),'total_credit' => $query->func()->sum('ReferenceDetails.credit')])
				->where(['ReferenceDetails.ledger_id'=>$contra_voucher_row->ledger_id,'ReferenceDetails.type !='=>'On Account'])
				...
				$refDropDown[$contra_voucher_row->id] = $option;
			}
		}
		
		$this->set(compact('contraVoucher', 'location_id','voucher_no','ledgerOptions', 'referenceDetails','city_id','refDropDown'));
