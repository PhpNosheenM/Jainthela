           ...
           if($isValidCity == 0)
           {
			   ...
			   $items = $this->AppMenus->Categories->find()
				->contain(['SellerItems' => function($q) use($city_id,$category_id) {
					return $q->where(['SellerItems.city_id'=>$city_id,'SellerItems.status'=>'Active'])
					->contain(['Items'=> function($q) {
						return $q->select(['id','name','category_id'])->where(['Items.status'=>'Active']);
					}, 'ItemVariations' => function ($q)
						{ return $q->select(['ItemVariations.seller_item_id','ItemVariations.id','ItemVariations.item_id','ItemVariations.ready_to_sale','ItemVariations.status'])
