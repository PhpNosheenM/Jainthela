...
<?php $this->set('title', 'Purchase Voucher'); ?>
<!-- PAGE CONTENT WRAPPER -->
<div class="page-content-wrap">
	<div class="row">
		<div class="col-md-12">
		<?= $this->Form->create($purchaseVoucher,['id'=>'jvalidate','class'=>'form-horizontal','type'=>'file']) ?>  
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title"><strong> Purchase Voucher</strong></h3>
				</div>
				...
				<div class="row">    
					...
					<div class="col-md-12">
							<div class="col-md-6">
								<div class="form-group">
									<label class="col-md-3 control-label"> Seller Invoice No </label>
									<div class="col-md-4">
										<?php echo $this->Form->input('seller_invoice_no', ['label' => false,'class' => 'form-control input-sm','placeholder'=>'Seller Invoice No.']); ?>	
									</div>
								</div>
							</div>
							...
							<div class="col-md-6">
								<div class="form-group">
									<label class="col-md-6 control-label">Seller Invoice Date</label>
									<div class="col-md-4">                     
										<?= $this->Form->control('seller_invoice_date',['class'=>'form-control datepicker','placeholder'=>'Transaction Date','label'=>false,'type'=>'text','data-date-format' => 'dd-mm-yyyy','value'=>date('d-m-Y')]) ?>
									</div>
								</div>
							</div>
						</div>
							...
							<table id="MainTable" class="table table-condensed table-striped" width="100%">
							  ...
							  //  unset($option_ref);								 
								...
								$option_mode[]= ['value'=>'NEFT/RTGS','text'=>'NEFT/RTGS'];
								 ...
								 {$i=0;		
								         foreach($purchaseVoucher->purchase_voucher_rows as $paymentRows)
									     { ?>
									
									<tr class="MainTr" row_no="<?php echo $i;?>">
										<td width="10%">
											<?php
											echo $this->Form->input('purchase_voucher_rows.'.$i.'.id',['value'=>$paymentRows->id,'class'=>'hidden']);
											if($i==0)
											{
												$options['Cr'] = 'Cr'; ?>
												<?= $this->Form->select('purchase_voucher_rows.'.$i.'.cr_dr',$options,['class'=>'form-control input-sm cr_dr select','label'=>false,'value'=>$paymentRows->cr_dr]) ?>
											<?php }
											else{
												 $options['Dr'] = 'Dr'; ?>
												<?= $this->Form->select('purchase_voucher_rows.'.$i.'.cr_dr',$options,['class'=>'form-control input-sm cr_dr select','label'=>false,'value'=>$paymentRows->cr_dr]) ?>
										...
										<td width="65%">
										<input type="hidden" class="BankValueDefine" name=" purchase_voucher_rows[<?php echo $i;?>][BankDefination]">
							
										<?php
										if($i==0)
										{ 
											echo $this->Form->select('purchase_voucher_rows.'.$i.'.ledger_id',$ledgerOptionFirst, ['empty'=>'--Select--','label' => false,'class' => 'form-control input-sm ledger select','required'=>'required', 'data-live-search'=>true,'value'=>$paymentRows->ledger_id]); ?>
										...
										{ 
											echo $this->Form->select('purchase_voucher_rows.'.$i.'.ledger_id',$ledgerOptionSecond, ['empty'=>'--Select--','label' => false,'class' => 'form-control input-sm ledger select','required'=>'required', 'data-live-search'=>true,'value'=>$paymentRows->ledger_id]); 
										}
										?>
											...
											?>
												...
												?>
													<tr>
														<td width="20%">
															<?php 
															echo $this->Form->input('purchase_voucher_rows.'.$i.'.reference_details.'.$j.'.ledger_id', ['type'=>'hidden','label' => false,'class' => 'form-control input-sm ledgerIdContainer','value'=>$reference_detail->ledger_id]); ?>
															
															
															
															<?php 
															echo $this->Form->input('purchase_voucher_rows.'.$i.'.reference_details.'.$j.'.city_id', ['type'=>'hidden','label' => false,'class' => 'form-control input-sm companyIdContainer','value'=>$reference_detail->city_id]); ?>
															 
															<?php
																echo $this->Form->select('purchase_voucher_rows.'.$i.'.reference_details.'.$j.'.type',$option_ref, ['label' => false,'class' => 'form-control input-sm refType select','required'=>'required','value'=>$reference_detail->type]); ?>
														</td>
														...
														?>
															...
															{
																echo $this->Form->input('purchase_voucher_rows.'.$i.'.reference_details.'.$j.'.ref_name', ['options'=>@$refDropDown[$paymentRows->id],'label' => false,'class' => 'form-control input-sm paymentType refList','required'=>'required','value'=>$reference_detail->ref_name]);
												...
												<tfoot>
												    <tr class="remove_ref_foot">
														...
														<td valign="top">
														<input type="text" class="form-control input-sm rightAligntextClass total calculation noBorder" name="purchase_voucher_rows[<?php echo $i;?>][total]" id="purchase_voucher_rows-<?php echo $i;?>-total" aria-invalid="true" aria-describedby="purchase_voucher_rows-<?php echo $i;?>-total-error" value="<?php echo $total;?>" readonly>
														</td>
														<td valign="top"><input type="text" class="form-control input-sm total_type calculation noBorder" readonly value="<?php echo @$type;?>" name="purchase_voucher_rows<?php echo $i;?>reference_details<?php echo $i;?>type_cr_dr"></td>
													</tr>
												</tfoot>
												</table>
											...
											<table width='90%'>
												<tbody>
													<tr>
														<td width="30%">
															<?php 
															echo $this->Form->input('purchase_voucher_rows.'.$i.'.mode_of_payment', ['options'=>$option_mode,'label' => false,'class' => 'form-control input-sm paymentType select','required'=>'required','value'=>$paymentRows->mode_of_payment]); ?>
														</td>
													...
													<?php if($paymentRows->mode_of_payment=='NEFT/RTGS'){?>
														...
														<td width="30%" style="<?php echo $style;?>">
															<?php echo $this->Form->input('purchase_voucher_rows.'.$i.'.cheque_no', ['label' =>false,'class' => 'form-control input-sm cheque_no','placeholder'=>'Cheque No','value'=>$paymentRows->cheque_no]); ?>
															
														</td>
														<td width="30%" style="<?php echo $style;?>">
															<?php 
															if(!empty($paymentRow->cheque_date)){
																$cheque_date=date('d-m-Y', strtotime($paymentRows->cheque_date)); 
															}else{
																$cheque_date='';
															}
															
															echo $this->Form->input('purchase_voucher_rows.'.$i.'.cheque_date', ['label' =>false,'class' => 'form-control input-sm date-picker cheque_date ','data-date-format'=>'dd-mm-yyyy','placeholder'=>'Cheque Date','value'=>$cheque_date,'type'=>'text']); ?>
														</td>
													</tr>
												</tbody>
												<tfoot>
										...
										<td width="10%">
											 <?php if($paymentRows->debit > 0){ ?>
											<?php echo $this->Form->input('purchase_voucher_rows.'.$i.'.debit', ['label' => false,'class' => 'form-control input-sm debitBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Debit','value'=>$paymentRows->debit, 'type'=>'text']); ?>
											 <?php }else{ ?>
											 <?php echo $this->Form->input('purchase_voucher_rows.'.$i.'.debit', ['label' => false,'class' => 'form-control input-sm debitBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Debit','value'=>'', 'type'=>'text','style'=>'display:none;']); ?>
											 <?php } ?>
										</td>
										<td width="10%">
											 <?php if($paymentRows->credit > 0){ ?>
											<?php echo $this->Form->input('purchase_voucher_rows.'.$i.'.credit', ['label' => false,'class' => 'form-control input-sm creditBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Credit','value'=>$paymentRows->credit, 'type'=>'text']); ?>
											  <?php }else{ ?>
												<?php echo $this->Form->input('purchase_voucher_rows.'.$i.'.credit', ['label' => false,'class' => 'form-control input-sm creditBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Credit','value'=>'', 'type'=>'text','style'=>'display:none;']); ?>
											  <?php } ?>
										</td>
...
<?php
	$kk='<input type="text" class="form-control input-sm 
	 ...
	 $js="var form1 = $('#jvalidate').validate({
		...
		function renameMainRows(){ 
				...
				$('#MainTable tbody#MainTbody tr.MainTr').each(function(){
					...
					$(this).find('td:nth-child(1) select.cr_dr').attr({name:'purchase_voucher_rows['+i+'][cr_dr]',id:'purchase_voucher_rows-'+i+'-cr_dr'}).selectpicker();
					$(this).find('td:nth-child(2) select.ledger').attr({name:'purchase_voucher_rows['+i+'][ledger_id]',id:'purchase_voucher_rows-'+i+'-ledger_id'}).selectpicker();
			...
			function renameBankRows(SelectedTr){
				...
				SelectedTr.find('td:nth-child(2) div.window table tbody tr').each(function(){
					...
					//alert(type);
					$(this).find('td:nth-child(1) select.paymentType').attr({name:'purchase_voucher_rows['+row_no+'][mode_of_payment]',id:'purchase_voucher_rows-'+row_no+'-mode_of_payment'}).selectpicker();
			...
			function renameRefRows(SelectedTr){
				...
				SelectedTr.find('td:nth-child(2) div.window table tbody tr').each(function(){
					$(this).find('td:nth-child(1) input.companyIdContainer').attr({name:'purchase_voucher_rows['+row_no+'][reference_details]['+i+'][location_id]',id:'purchase_voucher_rows-'+row_no+'-reference_details-'+i+'-location_id'});
					...
					$(this).find('td:nth-child(1) select.refType').attr({name:'purchase_voucher_rows['+row_no+'][reference_details]['+i+'][type]',id:'purchase_voucher_rows-'+row_no+'-reference_details-'+i+'-type'}).selectpicker();
					...
					if(Dr_Cr=='Dr'){
						$(this).find('td:nth-child(3) input').attr({name:'purchase_voucher_rows['+row_no+'][reference_details]['+i+'][debit]',id:'purchase_voucher_rows-'+row_no+'-reference_details-'+i+'-debit'}).rules('add', 'required');
					}else{
						$(this).find('td:nth-child(3) input').attr({name:'purchase_voucher_rows['+row_no+'][reference_details]['+i+'][credit]',id:'purchase_voucher_rows-'+row_no+'-reference_details-'+i+'-credit'}).rules('add', 'required');
					}
					i++;
				});
