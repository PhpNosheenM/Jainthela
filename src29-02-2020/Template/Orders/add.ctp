		...
		<?= $this->Form->create($order,['id'=>'jvalidate','class'=>'form-horizontal','type'=>'file']) ?>  
			...
			<?php //pr($sales_orders->customer_id); ?>
						...
						<div class="col-md-3">
							...
							<div class="form-group">
								...
								<div class="">  
									...
									<input type="hidden" name="customer_id" value="<?php echo @$sales_orders->customer_id; ?>">
										<input type="hidden" name="customer_address_id" value="<?php echo @$sales_orders->customer_address_id; ?>">
										<input type="hidden" class="membership_discount_percent" name="membership_discount_percent" value="<?php echo @$membership_discount_percent; ?>">
										
										<?= $this->Form->select('customer_id',$partyOptions,['empty'=>'---Select--','class'=>'form-control select prty_ldgr','label'=>false,'data-live-search'=>true,'value'=>$sales_orders->sales_ledger_id]) ?>
										
									<?php //echo $sales_orders->customer->name; ?>
									
									<?php }else if(!empty($CancelItemOrder)){ ?>
										<input type="hidden" name="customer_id" value="<?php echo @$CancelItemOrder->customer_id; ?>">
										<input type="hidden" name="customer_address_id" value="<?php echo @$CancelItemOrder->customer_address_id; ?>">
										
										<?php echo $CancelItemOrder->customer->name; ?>
									<?php }else{ ?>
										<?= $this->Form->select('customer_id',$partyOptions,['empty'=>'---Select--','class'=>'form-control select prty_ldgr','label'=>false,'data-live-search'=>true]) ?>
								...
								?>
									<tr class="MainTr">
										...
										<td  valign="top" class="itemList">
											...
											<?= $this->Form->control('combo_offer_id',['type'=>'hidden','class'=>'form-control cmb_ofr_id','label'=>false,'value'=>$sales_order_row->combo_offer_id]) ?>
										</td>
				...
				<div class="panel-footer">
					<center>
						<a class="btn btn-primary  btn-condensed all_calculation" role="button" onClick="calculationfinal()" >Calculation</a>
	...
	<tbody class="sampleMainTbody">
		<tr class="MainTr">
			...
			<td  valign="top" class="itemList"> 
				...
				<?= $this->Form->control('combo_offer_id',['type'=>'hidden','class'=>'form-control cmb_ofr_id','label'=>false,'value'=>0]) ?>
			</td>
			...
			<td valign="top">
				...
				<!--<?= $this->Form->control('promo_percent',['type'=>'hidden','class'=>'form-control promo_percent','label'=>false,'value'=>0]) ?>
			
				<?= $this->Form->control('promo_amount',['type'=>'hidden','class'=>'form-control promo_amount','label'=>false,'value'=>0,'readonly']) ?>-->
			</td>
			<td>
			<?= $this->Form->control('discount_amount',['class'=>'form-control discount_amount','label'=>false,'readonly']) ?>
			</td>
...
if(empty($ids) && empty($CancelItemOrder)){
   ...
   $js="var jvalidate = $('#jvalidate').validate({
		...
		//addcomboRow();
		/* 
		$(document).on('click','.all_calculation',function(){ alert(); 
			calculationfinal();
		});
		...
		$(document).on('change','.itemSel',function(){ 
			...
			//alert(put_stock); tot_discount_amount 
		...
		function renameRows(){
			...
			$('.main_table tbody tr').each(function(){
				...
				$(this).find('.original_rate ').attr({name:'order_details['+i+'][original_rate]',id:'order_details['+i+'][original_rate]'}).rules('add', 'required');
		...
		function calculation(thisObj){
			var tot_discount_amount=0;
			var discount_applicable=thisObj.find('option:selected', this).attr('discount_applicable');
					var member_dis=$('.prty_ldgr').find('option:selected').attr('membership_discount');
					thisObj.closest('tr').find('.discount_percent').val(member_dis);
					//var original_rate=$(this).find('.original_rate').val();
					var rate=thisObj.find('.rate').val();
					
					//$(this).closest('tr').find('.rate').val(original_rate);
					var qty=thisObj.find('.quantity').val();
					var gst_percentage=parseFloat(thisObj.find('.gst_percentage').val()); 
					
					var original_amount=qty*rate;
					var actual_amount=qty*rate;
					
					if(discount_applicable=='No'){
						thisObj.closest('tr').find('.discount_percent').val(0);
						thisObj.closest('tr').find('.discount_amount').val(0);
						thisObj.closest('tr').find('.discount_percent').attr('applicable','No');
						//var rate=original_rate;
					}else{
						thisObj.closest('tr').find('.discount_percent').attr('applicable','Yes');
						var dis=member_dis*original_amount/100;
						tot_discount_amount=tot_discount_amount+dis;
						thisObj.closest('tr').find('.discount_percent').val(round(member_dis,2));
						thisObj.closest('tr').find('.discount_amount').val(round(dis,2));
					...
					thisObj.find('.amount').val(round(actual_amount,2));
					if(!gst_percentage){ 
						gst_rate=0;
					}else{ 
					...
					thisObj.find('.gst_value').val(gst_rate);
					var net_amount=gst_rate+taxable_value;
					var net_amount1=round(net_amount,2);
					thisObj.find('.net_amount').val(round(net_amount,2));
