...
class ComboOffersController extends AppController
{
    ...
    public function add()
    {
		...
		$i=0; foreach($itemVariations as $itemVariation){
				$is_discount_enable=$itemVariation->item->is_discount_enable;
				if($is_discount_enable=='No'){
					continue;
				}
			$itemVariation_option[]=['text'=>$itemVariation->item->name .' ' .$itemVariation->unit_variation->quantity_variation .' '.$itemVariation->unit_variation->unit->unit_name,'value'=>$itemVariation->id,'rate'=>$itemVariation->sales_rate,'gst_per'=>@$itemVariation->item->gst_figure->tax_percentage, 'gst_figure_id'=>@$itemVariation->item->gst_figure->id ];
		}
	...
	public function edits($ids = null)
    {
		...
		$old_combo_offer_image=$comboOffer->combo_offer_image;
		$old_combo_offer_image_web=$comboOffer->combo_offer_image_web;
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            $comboOffer = $this->ComboOffers->patchEntity($comboOffer, $this->request->getData());
			$comboOffer->combo_offer_image_web=$old_combo_offer_image_web;
			$comboOffer->combo_offer_image=$old_combo_offer_image;
			if(empty($combo_offer_error))
			{
				$combo_offer_ext=explode('/',$combo_offer_image['type']);
				$combo_offer_image_name='combo_offer'.time().'.'.$combo_offer_ext[1];
