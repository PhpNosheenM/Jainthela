...
class WastageVouchersController extends AppController
{
    ...
    public function view($id = null)
    {
        ...
        $wastageVoucher = $this->WastageVouchers->get($id, [
            'contain' => ['Cities', 'Locations', 'WastageVoucherRows'=>['Items','ItemVariations'=>['UnitVariations'=>['Units']]]]
        ]);
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
			...
			$Voucher = $this->WastageVouchers->find()->select(['voucher_no'])->where(['city_id'=>$city_id,'financial_year_id'=>$financial_year_id])->order(['voucher_no' => 'DESC'])->first();
			if($Voucher)
			{
			...
			$wastageVoucher->financial_year_id = $financial_year_id;
		...
		$Brands = $this->WastageVouchers->WastageVoucherRows->Items->Brands->find('list');
		$Locations = $this->WastageVouchers->Locations->find('list')->where(['city_id'=>$city_id]);
		$this->set(compact('ItemsVariations','Locations','Cities','from_date','to_date','city_id','location_id','showItems','wastageVoucher','Brands','brand_id'));
    }
