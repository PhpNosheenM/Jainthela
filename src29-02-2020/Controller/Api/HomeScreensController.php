...
class HomeScreensController extends AppController
{
	...
	public function current_api_version(){
		...
		$is_ios=@$this->request->query['is_ios'];
		if(!empty($api_version)){
			
			if($is_ios == 'yes')
			{
				//$ApiVersions=$this->HomeScreens->ApiVersions->find()->where(['ios_verision'=>$api_version]);				
				$ApiVersions=$this->HomeScreens->ApiVersions->find();				
			}
			else
			{
				...
				foreach($HomeScreens as $HomeScreen){
					...
					if($HomeScreen->model_name=='Brands'){
						...
						if($HomeScreen->model_name=='Banners'){
								$Banners=$this->HomeScreens->Banners->find()->where(['city_id'=>$city_id,'status'=>'Active','last_date >=' => date('Y-m-d')]);
								if($Banners->toArray()){
					...
					if($HomeScreen->model_name=='Category'){
							...
							$Items->where(['status'=>'Active','city_id'=>$city_id,'id'=>$HomeScreen->category_id])->contain(['SellerItems'=> function($q) use($city_id,$Items) { 
								return $q->contain(['Items' => function($q) {
								...
								} ,'ItemRating' => function($q) use($Items) {
									...
									->leftJoinWith('AverageReviewRatings');
										...
										return $q->where(['ItemVariations.status'=>'Active'])->contain(['ItemVariationMasters','UnitVariations'=>['Units']]);
								}  ])
								...
								{	
									foreach ($itemData->seller_items as $Item) {
										...
										foreach ($Item->item_variations as $items_variation) {
											...
											$items_variation->maximum_quantity_purchase = round($items_variation->maximum_quantity_purchase);
											$cs = $items_variation->current_stock;
											$vs = $items_variation->virtual_stock;
											$ds = $items_variation->demand_stock;
											$mqp = $items_variation->maximum_quantity_purchase;
											...
											if($stock > $mqp)
											{
												$items_variation->maximum_quantity_purchase = round($mqp);
											}
											else if($mqp > $stock)
											{
												$items_variation->maximum_quantity_purchase = round($stock);
											}
											else {
												$items_variation->maximum_quantity_purchase = round($mqp);
											}
											...
											$inNotifylist = 0;
											$inNotifylist = $this->HomeScreens->Categories->Items->Notifies->find()
												->where(['item_id'=>$item_id,'item_variation_id'=>$items_variation_id])
												->where(['customer_id'=>$customer_id])->count();
											$items_variation->inNotifylist = false;
											//echo $items_variation_id;exit;
											if($inNotifylist  >= 1)
											{ $items_variation->inNotifylist = true; }
											else { $items_variation->inNotifylist = false; }
								...
								$total_item=0;
									...
									$inWishList = $this->HomeScreens->ComboOffers->WishListItems->find()->where(['combo_offer_id'=>$Combooffer->id])->contain(['WishLists'=>function($q) use($customer_id){
										...
										$Combooffer->inWishList = $inWishList;
											...
											$inNotifylist = 0;
											$inNotifylist = $this->HomeScreens->ComboOffers->Notifies->find()
												->where(['combo_offer_id'=>$Combooffer->id])
												->where(['customer_id'=>$customer_id])->count();
											$Combooffer->inNotifylist = false;
											if($inNotifylist  >= 1)
											{ $Combooffer->inNotifylist = true; }
											else { $Combooffer->inNotifylist = false; }
								...
								foreach ($Singleimagetwoitems as $Item) {
									foreach($Item->seller_items as $itemData)
									{
										...
										foreach($itemData->item_variations as $items_variation) {
											...
											$inWishList = $this->HomeScreens->Categories->Items->WishListItems->find()->where(['item_id'=>$itemData->item_id,'item_variation_id'=>$items_variation->id])->contain(['WishLists'=>function($q) use($customer_id){
											return $q->select(['WishLists.customer_id'])->where(['customer_id'=>$customer_id]);}])->count();
											$items_variation->inWishList = false;
											if($inWishList  >= 1)
											{ $items_variation->inWishList = true; }
											...
											$inNotifylist = 0;
											$inNotifylist = $this->HomeScreens->Categories->Items->Notifies->find()
												->where(['item_id'=>$itemData->item_id,'item_variation_id'=>$items_variation->id])
												->where(['customer_id'=>$customer_id])->count();
											$items_variation->inNotifylist = false;
											if($inNotifylist  >= 1)
											{ $items_variation->inNotifylist = true; }
