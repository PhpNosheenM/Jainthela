...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $locationItem->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $locationItem->id)]
            )
        ?></li>
...
<div class="locationItems form large-9 medium-8 columns content">
    <?= $this->Form->create($locationItem) ?>
    <fieldset>
        <legend><?= __('Edit Location Item') ?></legend>
        <?php
            echo $this->Form->control('item_id', ['options' => $items]);
            echo $this->Form->control('item_variation_master_id');
