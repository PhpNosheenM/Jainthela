...
</style><?php $this->set('title', 'Edit GRN'); ?>
<div class="page-content-wrap">
    <div class="page-title">                    
        <h2><span class="fa fa-arrow-circle-o-left"></span> GRN</h2>
    </div> 
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
			...
			<?php 
			$js="";
			echo $this->Form->input('grn_id', ['label' => false,'type'=>'hidden','value'=>$grn->id,'id'=>'grn_id']); ?>
                ...
                <div class="">
                    <table id="main_table" class="table table-condensed table-bordered" style="margin-bottom: 4px;" width="100%">
                        ...
                        <tbody id='main_tbody' class="tab">
							<?php $i=1;$j=1; foreach($grn->grn_rows as $grn_row){ ?>
							<tr class="main_tr tab<?php echo $j; ?>">
								<td width="">
									<?php echo $i++; ?>
								</td>
								<td width="">
									<?php echo $this->Form->input('id', ['label' => false,'type'=>'hidden','value'=>$grn_row->id,'class'=>'row_id']); ?>

									<?php echo $this->Form->select('item_id',$itemOptions, ['empty'=>'--Select--','label' => false,'class' => 'form-control input-sm item_id select','required'=>'required', 'data-live-search'=>true,'value'=>$grn_row->item_id]); ?>
										

									
								</td>
								<td id="unit_variations">
									<?php //echo $grn_row->unit_variation->visible_variation; ?>
									<?php //echo $this->Form->input('unit_variation_id', ['empty'=>'---Select---','options'=>$unitVariationOptions,'label' => false,'class' => 'form-control input-medium unit_variation_id','required'=>'required','value'=>$grn_row->unit_variation_id]); ?>
									<?php
											$js.="var item_id='".$grn_row->item_id."';
											var unit_variation_id='".$grn_row->unit_variation_id."';
													
													var url='".$this->Url->build(["controller" => "Items", "action" => "SelectItemVariation"])."';
													url=url+'/'+item_id+'/'+unit_variation_id;

													$.ajax({
														url: url,
														type: 'GET'
													}).done(function(response) {
														$('.tab".$j++."').find('#unit_variations').html(response);
													});";
										?>
								</td>
								<td width="" >
									<?php echo $this->Form->input('quantity', ['label' => false,'class' => 'form-control input-sm numberOnly rightAligntextClass total quantity','placeholder'=>'Qty','required'=>true,'value'=>$grn_row->quantity]); ?>
								</td>
								<td width="">
									<?php echo $this->Form->input('rate', ['label' => false,'class' => 'form-control input-sm total numberOnly rightAligntextClass rate','required'=>'required','placeholder'=>'Purchase Rate','required','value'=>$grn_row->rate]); ?> 
								</td>
								<td width="">
									<?php echo $this->Form->input('amount', ['label' => false,'class' => 'form-control input-sm total numberOnly rightAligntextClass amount','required'=>'required','placeholder'=>'Purchase Amount','required','value'=>$grn_row->amount,'type' => 'hidden']); ?> 
									
									<?php echo $this->Form->input('taxable_value', ['label' => false,'class' => 'form-control input-sm total numberOnly rightAligntextClass taxable_value','required'=>'required','placeholder'=>'Taxable','required','readonly','value'=>$grn_row->taxable_value]); ?> 
									
								</td>
								<td width="">
									<?php echo $this->Form->input('gst_value', ['label' => false,'class' => 'form-control input-sm total numberOnly rightAligntextClass gst_value','required'=>'required','placeholder'=>'Purchase Rate','required','value'=>$grn_row->gst_value]); ?> 
								</td>
								<td width="">
									<?php echo $this->Form->input('net_amount', ['label' => false,'class' => 'form-control input-sm total numberOnly rightAligntextClass net_amount','required'=>'required','placeholder'=>'','required','value'=>$grn_row->net_amount]); ?> 
									<?php echo $this->Form->input('purchase_rate', ['type' => 'hidden','label' => false,'class' => 'form-control input-sm total numberOnly rightAligntextClass purchase_rate','required'=>'required','placeholder'=>'Purchase Rate','required','value'=>$grn_row->purchase_rate]); ?> 
									<?php //echo $this->Form->input('gst_value', ['label' => false,'class' => 'form-control input-sm total numberOnly rightAligntextClass gst_value','required'=>'required','placeholder'=>'Purchase Rate','required','type' => 'hidden','value'=>$grn_row->gst_value]); ?> 
								</td>
								<td width="">
								...
								<td align="center">
									<a class="btn btn-danger delete-tr btn-xs" href="#" role="button" style="margin-bottom: 5px;" grn_id="<?php echo $grn_row->grn_id; ?>" grn_row_id="<?php echo $grn_row->id; ?>"><i class="fa fa-times"></i></a>
									<?= $this->Form->select('gst_type',$GstType,['class'=>'form-control  gst_type','label'=>false,'value'=>$grn_row->gst_type]) ?>
									 <?php echo $this->Form->input('net_amount', ['label' => false,'class' => 'form-control input-sm total numberOnly rightAligntextClass net_amount','required'=>'required','placeholder'=>'','required','style'=>'display:none','value'=>$grn_row->net_amount]); ?> 
								</td>
							</tr>
							<?php } ?>
                        </tbody>
...
<table id="sample_table" style="display:none;" width="100%">
 ...
 <tr class="main_tr" class="tab">
           ...
           <td>
			...
			<td width="">
				<?php echo $this->Form->control('expiry_date',['class'=>'form-control temp datepicker expiry_date','data-date-format'=>'dd-mm-yyyy', 'label'=>false,'placeholder'=>'DD-MM-YYYY','type'=>'text','data-date-start-date'=>@$coreVariable[fyValidFrom],'data-date-end-date'=>@$coreVariable[fyValidTo],'value'=>date('d-m-Y'),'data-date-format' => 'dd-mm-yyyy','required'=>'required']); ?>
            </td>
...
<?php
    ...
    $(document).ready(function() {
         $(document).on('click','.delete-tr',function() 
        {
			...
			{ 
				var grn_row_id=$(this).attr('grn_row_id');
				var url='".$this->Url->build(["controller" => "Grns", "action" => "deleteGrnRow"])."';
				url=url+'/'+grn_row_id;
				$.ajax({
					url: url,
					type: 'GET'
        ...
        $(document).on('submit','#jvalidate',function(e){
			...
			$('input[name=total_purchase_rate]').val(round(total_purchase,2));
			 ...
			 var urlgrn = '".$this->Url->build(['action'=>'editGrn.json'])."';
                ...
                $.ajax({
		            ...
		            type: 'get',
		            dataType: 'json',
		            data: {grn_id:grn_id,vendor_ledger_id:vendor_ledger_id,transaction_date:transaction_date,total_purchase_rate:total_purchase_rate},
		            success: function(result)
		            {
		            	
		                //var obj = JSON.parse(JSON.stringify(result));
		                //console.log(obj.response);
		               var url = '".$this->Url->build(['action'=>'editGrnRow.json'])."';
		                var i=0;
			            $('#main_table tbody#main_tbody tr.main_tr').each(function()
			            {
							$(this).find('.row_id').attr({name:'grn_rows['+i+'][id]', id:'grn_rows-'+i+'-id'});
							...
							// $(this).find('.gst_type').selectpicker();
			                ...
			                $.ajax({
					            ...
					            type: 'get',
					            dataType: 'json',
					            data: {id:id,item_id:item_id,unit_variation_id:unit_variation_id,quantity:quantity,amount:amount,taxable_value:taxable_value,rate:rate,purchase_rate:purchase_rate,net_amount:net_amount,gst_type:gst_type,expiry_date:expiry_date,grn_id:grn_id,gst_value:gst_value},
					            success: function(result)
					            {
					                var obj = JSON.parse(JSON.stringify(result));
