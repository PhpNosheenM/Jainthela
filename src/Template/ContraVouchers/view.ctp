		...
		<table width="100%" class="table" style="font-size:12px">
			...
			<?php foreach($contraVoucher->contra_voucher_rows as $contra_voucher_row)
				{ 
					...
					<tr>
					<td colspan="3" style="text-align:left"><b><?=$contra_voucher_row->ledger->name?>
					
					<?php if($contra_voucher_row->mode_of_payment=='Cheque'){  
						echo '( Cheque no-'.$contra_voucher_row->cheque_no.', Date '.date('d-M-Y', strtotime($contra_voucher_row->cheque_date)).')';
					} ?>
					</b>
