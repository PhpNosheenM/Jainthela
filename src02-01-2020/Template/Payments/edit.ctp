...
<?php $this->set('title', 'Payment Voucher'); ?>
<!-- PAGE CONTENT WRAPPER -->
<div class="page-content-wrap">
	<div class="row">
		<div class="col-md-12">
		<?= $this->Form->create($payment,['id'=>'jvalidate','class'=>'form-horizontal','type'=>'file']) ?>  
								 ...
								 {$i=0;		
									...
									<tr class="MainTr" row_no="<?php echo $i;?>">
										<td width="10%">
											...
											{
												$options['Dr'] = 'Dr'; ?>
												<?= $this->Form->select('payment_rows.'.$i.'.cr_dr',$options,['class'=>'form-control input-sm cr_dr select','label'=>false,'value'=>$paymentRows->cr_dr]) ?>
											<?php }
											else{
												 $options['Dr'] = 'Dr'; 
												 $options['Cr'] = 'Cr'; ?>
										...
										if($i==0)
										{ 
											echo $this->Form->select('payment_rows.'.$i.'.ledger_id',$ledgerOptions, ['empty'=>'--Select--','label' => false,'class' => 'form-control input-sm ledger select','required'=>'required', 'data-live-search'=>true,'value'=>$paymentRows->ledger_id]); ?>
												...
												?>
													<tr>
														<td width="20%">
															...
															<?php 
															echo $this->Form->input('payment_rows.'.$i.'.reference_details.'.$j.'.city_id', ['type'=>'hidden','label' => false,'class' => 'form-control input-sm companyIdContainer','value'=>$reference_detail->city_id]); ?>
															 
															<?php
																echo $this->Form->select('payment_rows.'.$i.'.reference_details.'.$j.'.type',$option_ref, ['label' => false,'class' => 'form-control input-sm refType select','required'=>'required','value'=>$reference_detail->type]); ?>
														</td>
											...
											<table width='90%'>
												<tbody>
													<tr>
														<td width="30%">
															<?php 
															echo $this->Form->input('payment_rows.'.$i.'.mode_of_payment', ['options'=>$option_mode,'label' => false,'class' => 'form-control input-sm paymentType select','required'=>'required','value'=>$paymentRows->mode_of_payment]); ?>
														</td>
													...
													<?php if($paymentRows->mode_of_payment=='NEFT/RTGS'){?>
														...
														<td width="30%" style="<?php echo $style;?>">
															...
															echo $this->Form->input('payment_rows.'.$i.'.cheque_date', ['label' =>false,'class' => 'form-control input-sm date-picker cheque_date ','data-date-format'=>'dd-mm-yyyy','placeholder'=>'Cheque Date','value'=>$cheque_date,'type'=>'text']); ?>
														</td>
													</tr>
												</tbody>
												<tfoot>
										...
										<td width="10%">
											...
											<?php echo $this->Form->input('payment_rows.'.$i.'.debit', ['label' => false,'class' => 'form-control input-sm debitBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Debit','value'=>$paymentRows->debit, 'type'=>'text']); ?>
											 <?php }else{ ?>
											 <?php echo $this->Form->input('payment_rows.'.$i.'.debit', ['label' => false,'class' => 'form-control input-sm debitBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Debit','value'=>'', 'type'=>'text','style'=>'display:none;']); ?>
											 <?php } ?>
										</td>
										<td width="10%">
											...
											<?php echo $this->Form->input('payment_rows.'.$i.'.credit', ['label' => false,'class' => 'form-control input-sm creditBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Credit','value'=>$paymentRows->credit, 'type'=>'text']); ?>
											  <?php }else{ ?>
												<?php echo $this->Form->input('payment_rows.'.$i.'.credit', ['label' => false,'class' => 'form-control input-sm creditBox rightAligntextClass numberOnly totalCalculation calculate_total','placeholder'=>'Credit','value'=>'', 'type'=>'text','style'=>'display:none;']); ?>
											  <?php } ?>
										</td>
	 ...
	 $js="var form1 = $('#jvalidate').validate({
		...
		function renameMainRows(){ 
				...
				$('#MainTable tbody#MainTbody tr.MainTr').each(function(){
					...
					$(this).find('td:nth-child(1) select.cr_dr').attr({name:'payment_rows['+i+'][cr_dr]',id:'payment_rows-'+i+'-cr_dr'}).selectpicker();
					$(this).find('td:nth-child(2) select.ledger').attr({name:'payment_rows['+i+'][ledger_id]',id:'payment_rows-'+i+'-ledger_id'}).selectpicker();
			...
			function renameBankRows(SelectedTr){
				...
				SelectedTr.find('td:nth-child(2) div.window table tbody tr').each(function(){
					...
					//alert(type);
					$(this).find('td:nth-child(1) select.paymentType').attr({name:'payment_rows['+row_no+'][mode_of_payment]',id:'payment_rows-'+row_no+'-mode_of_payment'}).selectpicker();
			...
			function renameRefRows(SelectedTr){
				...
				SelectedTr.find('td:nth-child(2) div.window table tbody tr').each(function(){
					$(this).find('td:nth-child(1) input.companyIdContainer').attr({name:'payment_rows['+row_no+'][reference_details]['+i+'][location_id]',id:'payment_rows-'+row_no+'-reference_details-'+i+'-location_id'});
					...
					$(this).find('td:nth-child(1) select.refType').attr({name:'payment_rows['+row_no+'][reference_details]['+i+'][type]',id:'payment_rows-'+row_no+'-reference_details-'+i+'-type'}).selectpicker();
