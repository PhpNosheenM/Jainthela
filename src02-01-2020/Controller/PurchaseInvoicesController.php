...
class PurchaseInvoicesController extends AppController
{
    ...
    public function index()
    {
		...
		$purchase_invoices=$this->PurchaseInvoices->find()->where(['PurchaseInvoices.city_id'=>$city_id])->contain(['PurchaseReturns','PurchaseInvoiceRows'=>['GrnRows'=>function($p) {
	...
	$purchaseInvoices = $this->paginate($this->PurchaseInvoices->find()->contain(['PurchaseReturns'])->where(['PurchaseInvoices.city_id'=>$city_id]));
    ...
    public function view($id = null)
    {
        ...
        $purchaseInvoice = $this->PurchaseInvoices->get($id, [
            'contain' => ['SellerLedgers', 'PurchaseLedgers', 'Cities', 'PurchaseInvoiceRows'=>['UnitVariations','GstFigures','Items'], 'PurchaseReturns', 'ReferenceDetails']
        ]);
		$this->loadmodel('Companies');
		$company_details=$this->Companies->find()->where(['Companies.city_id'=>$city_id])->first();
		$this->set(compact('purchaseInvoice','company_details'));
   ...
   public function add($to_be_send=null)
    {
		...
		$Voucher_no = $this->PurchaseInvoices->find()->select(['voucher_no'])->where(['PurchaseInvoices.city_id'=>$city_id,'PurchaseInvoices.financial_year_id'=>$financial_year_id])->order(['voucher_no' => 'DESC'])->first();
        ...
        if ($this->request->is('post')) {
			...
			//Accounting Entries for Freight Amount//
			if($purchaseInvoice->freight_amount > 0){
				$TransportLedger = $this->PurchaseInvoices->AccountingGroups->Ledgers->find()->where(['Ledgers.transport' =>'Paid','Ledgers.city_id'=>$city_id])->first();
				$AccountingEntrie = $this->PurchaseInvoices->AccountingEntries->newEntity(); 
				$AccountingEntrie->ledger_id=$TransportLedger->id;
				$AccountingEntrie->debit=$purchaseInvoice->freight_amount;
				$AccountingEntrie->credit=0;
				$AccountingEntrie->transaction_date=$purchaseInvoice->transaction_date;
				//$AccountingEntrie->location_id=$location_id;
				$AccountingEntrie->city_id=$city_id;
...
//pr($partyParentGroups->toArray()); exit;
    ...
    public function delete($id = null)
    {
        ...
        $purchaseInvoice = $this->PurchaseInvoices->get($id,['contain'=>['PurchaseInvoiceRows'=>['GrnRows'=>function($q){
			return $q->select(['grn_id'])->group('grn_id');
		}]]]);
		foreach($purchaseInvoice->purchase_invoice_rows as $data){
			$query = $this->PurchaseInvoices->PurchaseInvoiceRows->GrnRows->Grns->query();
			$query->update()
			->set(['purchase_invoice_status'=>'Pending'])
			->where(['id'=>$data->grn_row->grn_id])
			->execute(); 
			
			//pr($data->grn_row->grn_id); exit;
		}
		$this->PurchaseInvoices->PurchaseInvoiceRows->deleteAll(['purchase_invoice_id' => $id]);
		$this->PurchaseInvoices->AccountingEntries->deleteAll(['purchase_invoice_id' => $id]);
