	...
	<table width="100%" class="divHeader">
		...
		<tr>
			<td width="50%" valign="top" align="left">
				<table>
					<tr>
						<td>Seller Invoice No</td>
						<td width="20" align="center">:</td>
						<td><?= h(str_pad($purchaseVoucher->seller_invoice_no, 4, '0', STR_PAD_LEFT)) ?></td>
					</tr>
				</table>
			</td>
			<td width="50%" valign="top" align="right">
				<table>
					<tr>
						<td>Seller Invoice Date</td>
						<td width="20" align="center">:</td>
						<td><?= h(date("d-m-Y",strtotime($purchaseVoucher->seller_invoice_date))) ?></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<table width="100%" class="table" style="font-size:12px">
			...
			<?php foreach($purchaseVoucher->purchase_voucher_rows as $payment_row)
				{ 
					@$total_debit+=$payment_row->debit;
					@$total_credit+=$payment_row->credit; ?>
					<tr>
					<td colspan="3" style="text-align:left"><b><?=$payment_row->ledger->name?>
					
					<?php if($payment_row->mode_of_payment=='Cheque'){  
						echo '( Cheque no-'.$payment_row->cheque_no.', Date '.date('d-M-Y', strtotime($payment_row->cheque_date)).')';
					} ?>
					</b>
