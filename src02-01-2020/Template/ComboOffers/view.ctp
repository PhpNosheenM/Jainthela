...
<div style="border:solid 1px #c7c7c7;background-color: #FFF;padding: 10px;margin: auto;width: 55%;font-size: 12px;" class="maindiv">	
			...
			<td width="50%" valign="top" align="right">
				<table>
					<tr>
						<td>Print rate</td>
						<td width="20" align="center">:</td>
						 <td><strike><?= h($comboOffers->print_rate) ?></strike></td>
					</tr>
					...
					<tr>
						<td>Sales rate</td>
						<td width="20" align="center">:</td>
						<td ><b><?= h($comboOffers->sales_rate) ?></b></td>
	...
	<table width="100%" class="table" style="font-size:12px" align="center">
		<tr >
			...
			<td ><strong><?= __('Taxable') ?></strong></td>
			<td ><strong><?= __('GST %') ?></strong></td>
			<td ><strong><?= __('GST Amount') ?></strong></td>
		...
		foreach ($comboOffers->combo_offer_details as $data){
			...
			$show_item=$item_name.' ('.$alise_name.') -'.$unit_name.' ('.$quantity_variation.')';
			...
			<td ><?=h(++$i)?></td>
			<td ><?=h($show_item) ?></td>
			<td ><?=h($data->taxable_value)?></td>
			<td ><?=h($data->gst_percentage)?></td>
			<td ><?=h($data->gst_value)?></td>
			<td ><?=h($data->rate)?></td>
