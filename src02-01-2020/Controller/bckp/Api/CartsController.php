				...
				if($tag=='add')
					...
					//pr($comboData->toArray());exit;
						...
						if(!empty($categories->toArray()))
						{
							...
							->where(['Carts.customer_id' => $customer_id])
							->where(['Carts.city_id' =>$city_id])
							->contain(['ItemVariations'=>['ItemVariationMasters','Items' =>['GstFigures','Categories'] ,'UnitVariations'=>['Units']]])
							->group('Carts.item_variation_id')->autoFields(true)->toArray();
					...
					// Membership discount start			
									...
									foreach($combo->combo_offer->combo_offer_details as $combo_offer_detail)
									{
									   ...
									   $cate_id = $comboItemDetails->item->category_id;
										if($cash_back == 0){
											...
											$promoDiscount_Amount = $promoDiscount_Amount + $combo_offer_detail->promoDiscount_Amount;
										...
										$combo->promoDiscount_Amount = number_format($promoDiscount_Amount,2,".","");
						...
						{  
							foreach($comboData as $combo)
							{
								...
								foreach($combo->combo_offer->combo_offer_details as $combo_offer_detail)
								{
									$Combo_Promo_Total = number_format($Combo_Promo_Total + $combo_offer_detail->promoDiscount_in_Amount,2,".","");
						...
						$PromoTotal = 0.00;
						$PromoTotal = number_format($Item_Promo_total + $Combo_Promo_Total,2,".","");
						...
						//pr($carts_data);pr($comboData);exit;		
					if(empty($carts_data) && empty($comboData))
					{
					  $success = false;
					  $message ='Empty Cart';
					...
					else
					{
					  $success = true;
					  $message = 'Cart Data found'; 
					  $this->set(compact('success', 'message','isPromoValidForAll','isPromoValidForAllMsg','Round_off_amt','address_available','delivery_charge_amount','payableAmount','remaining_wallet_amount','carts','item_in_cart','comboData','Combototal','promo_detail_id','ItemWiseTotal_taxable','ItemWiseTotal_With_GST','Combototal_with_GST','Sub_Total','PromoTotal','MembershipTotal','Total_GST'));
					  $this->set('_serialize', ['success','message','isPromoValidForAll','isPromoValidForAllMsg','promo_detail_id','remaining_wallet_amount','item_in_cart','address_available','carts','ItemWiseTotal_taxable','ItemWiseTotal_With_GST','comboData','Combototal','Combototal_with_GST','Sub_Total','PromoTotal','MembershipTotal','Total_GST','delivery_charge_amount','Round_off_amt','payableAmount']);
					}
...
public function reviewOrder()
{
	...
	$cancel_order_limit = $this->Carts->MasterSetups->find()->select(['cancel_order_limit','online_amount_limit'])
...
/* 												$item_id = $cart_data->item_variation->item->id;
	    ...
	    $this->set(compact('success', 'message','order_no','isPromoValidForAll','isPromoValidForAllMsg','online_amount_limit','OrderCancelLimit','CustomerOrderCount',
		'remaining_wallet_amount','item_in_cart','delivery_charge_amount','payableAmount',
		'customer_addresses','all_dates','carts','comboData','Combototal',
		'promo_detail_id','holidays_date','ItemWiseTotal_taxable','ItemWiseTotal_With_GST','Combototal_with_GST','Sub_Total',
	  'PromoTotal','MembershipTotal','Total_GST','Round_off_amt'));
		...
		'carts','ItemWiseTotal_taxable','ItemWiseTotal_With_GST','comboData','Combototal','Combototal_with_GST','Sub_Total',
	  'PromoTotal','MembershipTotal','Total_GST','Round_off_amt','payableAmount']);
	}
