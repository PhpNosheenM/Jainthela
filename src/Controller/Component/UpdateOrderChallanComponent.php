				...
				$cancelChallan="Yes";
					foreach($challan->challan_rows as $challan_row){
						if($challan_row->is_item_cancel=="No"){
							...
							$gw_total+=$challan_row->wallet_amount;
							$cancelChallan="No";
						}
					}
					...
					if($cancelChallan=="No"){  
						...
						$cod_amount=$new_pay_amount-$gw_total;
						
						$query = $this->Challans->query();
						$query->update()
								->set(['grand_total' => $after_round_of,'discount_amount' => $total_discount_amount,'delivery_charge_amount' => $challan->delivery_charge_amount,'total_gst' => $total_gst,'total_amount' => $total_taxable,'round_off' => $round_off,'pay_amount' => $after_round_of+$challan->delivery_charge_amount,'old_pay_amount'=>$gw_total,'take_cod_amount'=>$cod_amount])
								->where(['id'=>$challan->id])
								->execute();
					}else{ 
						if(!empty($status))
